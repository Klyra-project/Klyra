...
use klyra_common::DeploymentApiError;
...
impl UserDirectory {
    ...
    fn users_toml_file_path() -> PathBuf {
        match std::env::var("klyra_USERS_TOML") {
            Ok(val) => val.into(),
            Err(_) => {
                log::debug!("could not find environment variable `klyra_USERS_TOML`, defaulting to MANIFEST_DIR");
