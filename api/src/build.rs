...
use klyra_service::loader::build_crate;

#[cfg(debug_assertions)]
pub const DEFAULT_FS_ROOT: &str = "/tmp/klyra/crates/";

#[cfg(not(debug_assertions))]
// as per: https://stackoverflow.com/questions/1510104/where-to-store-application-data-non-user-specific-on-linux
pub const DEFAULT_FS_ROOT: &str = "/var/lib/klyra/crates/";
...
impl BuildSystem for FsBuildSystem {
    async fn build(
        ...
        // check klyra service version of service
        check_klyra_version(&project_path, version_req)?;
...
fn create_so_marker(project_path: &Path, so_path: &Path) -> Result<()> {
    let marker_path = project_path.join(".klyra_marker");
...
fn check_klyra_version<P: AsRef<Path>>(
    working_directory: P,
    version_req: &VersionReq,
) -> anyhow::Result<()> {
    let manifest_path = working_directory.as_ref().join("Cargo.toml");
    let target_klyra_version = get_klyra_service_from_user_crate(&manifest_path)?;

    if version_req.matches(&target_klyra_version) {
        Ok(())
    } else {
        Err(anyhow!(
            "the version of `klyra-service` specified as a dependency to this service (v{target_klyra_version}) is not supported by this instance ({version_req}); try updating `klyra-service` to the latest version available and deploy"
        ))
    }
}
...
pub mod tests {
    ...
    fn our_version_is_accepted() {
        let version_req = VersionReq::parse(env!("klyra_SERVICE_VERSION_REQ")).unwrap();
        assert!(check_klyra_version("./", &version_req).is_ok())
    }
}
