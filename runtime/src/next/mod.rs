...
use klyra_common::wasm::{Bytesable, Log, RequestWrapper, ResponseWrapper};
use klyra_proto::runtime::runtime_server::Runtime;
...
impl Runtime for AxumWasm {
    async fn load(
        ...
        trace!(wasm_path, "loading klyra-next project");
...
impl Router {
    ...
    async fn handle_request(
        ...
        self.linker
            .get(&mut store, "axum", "__klyra_Axum_call")
...
pub mod tests {
    ...
    async fn axum() {
        ...
        let router = RouterBuilder::new()
            .unwrap()
            .src("tests/resources/axum-wasm-expanded/target/wasm32-wasi/debug/klyra_axum_expanded.wasm")
            .build()
            .unwrap();
