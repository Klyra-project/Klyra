...
pub fn resource(crate_name: &'static str, package_version: &'static str) -> Resource {
    let project_name = std::env::var("klyra_PROJECT_NAME").ok();
    ...
    TelemetryResourceDetector
        ...
        .merge(Box::new(Resource::from_schema_url(
            [
                ...
                Some(KeyValue::new("klyra.project.crate.name", crate_name)),
                Some(KeyValue::new(
                    "klyra.deployment.env",
                    std::env::var("klyra_ENV")
                        .ok()
                        .unwrap_or("unknown".into()),
                )),
                std::env::var("klyra_PROJECT_ID")
                    .ok()
                    .map(|value| KeyValue::new("klyra.project.id", value)),
                project_name.map(|value| KeyValue::new("klyra.project.name", value)),
            ]
            .into_iter()
            .flatten(),
            SCHEMA_URL,
...
pub fn init_tracing_subscriber(
    ...
    let layers = EnvFilter::from(&level_filter)
        ...
        .and_then(OpenTelemetryLayer::new(tracer.tracer("klyra-telemetry")))
        .and_then(
        ...
        .and_then(LogCourier::new(logger.logger("klyra-telemetry")));
