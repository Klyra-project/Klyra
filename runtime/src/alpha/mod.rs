...
use klyra_service::{ResourceFactory, Service};
use tokio::sync::{
...
pub async fn start(loader: impl Loader + Send + 'static, runner: impl Runner + Send + 'static) {
    ...
    let args = match Args::parse() {
        ...
        Err(e) => {
            ...
            let help_str = "[HINT]: Run klyra with `cargo klyra run`";
    ...
    println!(
        "klyra-runtime executable started (version {})",
        crate::VERSION
    );
    ...
    #[cfg(feature = "setup-tracing")]
    {
        ...
        tracing_subscriber::registry()
            ...
            .with(
                ...
                tracing_subscriber::EnvFilter::try_from_default_env()
                    ...
                    .or_else(|_| tracing_subscriber::EnvFilter::try_new("info,klyra=trace"))
                    .unwrap(),
            )
            .init();
        ...
        println!(
            ...
            {}",
            "=".repeat(63).yellow(),
            "Klyra's default tracing subscriber is initialized!"
                .yellow()
                .bold(),
            "klyra-runtime".italic(),
            r#"klyra-runtime = { version = "...", default-features = false }"#.italic(),
            "=".repeat(63).yellow(),
        );
    }
...
#[async_trait]
pub trait Loader {
    async fn load(self, factory: ResourceFactory) -> Result<Vec<Vec<u8>>, klyra_service::Error>;
}
...
where
    ...
    O: Future<Output = Result<Vec<Vec<u8>>, klyra_service::Error>> + Send,
{
    async fn load(self, factory: ResourceFactory) -> Result<Vec<Vec<u8>>, klyra_service::Error> {
        (self)(factory).await
    }
...
pub trait Runner {
    ...
    async fn run(self, resources: Vec<Vec<u8>>) -> Result<Self::Service, klyra_service::Error>;
}
...
where
    ...
    O: Future<Output = Result<S, klyra_service::Error>> + Send,
    S: Service,
{
    type Service = S;
