...
pub async fn start(loader: impl Loader + Send + 'static, runner: impl Runner + Send + 'static) {
    ...
    #[cfg(feature = "setup-tracing")]
    {
        ...
        if args.beta {
            tracing::warn!(
                "Default tracing subscriber initialized (https://docs.klyra.dev/docs/logs)"
            );
        } else {
            tracing::warn!("Default tracing subscriber initialized (https://docs.klyra.rs/configuration/logs)");
        }
    }
