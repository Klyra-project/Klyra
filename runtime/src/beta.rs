...
use klyra_service::{Environment, ResourceFactory, Service, KlyraResourceOutput};
...
struct BetaEnvArgs {
    ...
    /// Where to reach the required Klyra API endpoints (mainly for provisioning)
    api_url: String,
...
impl BetaEnvArgs {
    ...
    fn parse() -> Self {
        Self {
            klyra: std::env::var("klyra").is_ok(),
            project_id: std::env::var("klyra_PROJECT_ID").expect("project id env var"),
            project_name: std::env::var("klyra_PROJECT_NAME").expect("project name env var"),
            env: std::env::var("klyra_ENV")
                .expect("klyra environment env var")
                .parse()
                .expect("invalid klyra environment"),
            ip: std::env::var("klyra_RUNTIME_IP")
                .expect("runtime ip env var")
                .parse()
                .expect("invalid ip"),
            port: std::env::var("klyra_RUNTIME_PORT")
                .expect("runtime port env var")
                .parse()
                .expect("invalid port"),
            healthz_port: std::env::var("klyra_HEALTHZ_PORT")
                .map(|s| s.parse().expect("invalid healthz port"))
                .ok(),
            api_url: std::env::var("klyra_API").expect("api url env var"),
            api_key: std::env::var("klyra_API_KEY").ok(),
        }
    }
}
...
pub async fn start(loader: impl Loader + Send + 'static, runner: impl Runner + Send + 'static) {
    ...
    let client = KlyraApiClient::new(api_url, api_key, None);
    ...
    for (bytes, klyra_resource) in resources
        .iter_mut()
        .zip(values)
        ...
        })
    {
        ...
        if klyra_resource.r#type == Type::Secrets {
        ...
        println!("Provisioning {}", klyra_resource.r#type);
        loop {
            match client
                .provision_resource_beta(&project_id, klyra_resource.clone())
                .await
            {
    ...
    if klyra {
        ...
        let _ = client.client.get("/__klyra/shutdown").send().await;
    }
