...
- `cargo-klyra` is the CLI used by users to initialize, run, deploy and manage their projects and services on Klyra.

### Libraries

- `api-client` is a reqwest client for calling the Klyra API.
...
- `services` contains implementations of `Service` for common Rust frameworks to run on Klyra. Anything implementing `Service` can be deployed on Klyra.
...
```bash
cargo run --bin klyra -- --wd examples/rocket/hello-world run
```
