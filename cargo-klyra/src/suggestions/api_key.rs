...
pub fn reset_api_key_failed(err: anyhow::Error) -> anyhow::Error {
    ...
    println!();
    println!("If trying to log out and reset the API key at the same time fails repeteadly, please check Klyra status at https://status.klyra.rs or open a help thread on the Discord server.");
    err
}
