...
use klyra_service::database::Type;
...
impl LocalProvisioner {
    ...
    async fn get_container(
        ...
        match self.docker.inspect_container(container_name, None).await {
            ...
            Err(error) => {
                ...
                error!(
                    "Make sure Docker is installed and running. For more help: https://docs.klyra.dev/docs/local-run#docker-engines"
                );
    ...
    async fn get_db_connection_string(
        ...
        let container_name = format!("klyra_{project_name}_{type}");
    ...
    pub async fn start_container(
        ...
        let container_name = format!("klyra_{project_name}_{container_name}");
...
pub mod beta {
    ...
    use klyra_proto::provisioner::{provisioner_server::Provisioner, DatabaseRequest};
    use tonic::Request;
    use tracing::debug;
    ...
    async fn provision(
        ...
        Ok(match (method, uri) {
            ...
            (Method::POST, "/projects/proj_LOCAL/resources") => {
                ...
                let klyra_resource: ProvisionResourceRequestBeta =
                    serde_json::from_slice(&body).context("deserializing resource request")?;

                let response = match klyra_resource.r#type {
                    ...
                    | ResourceTypeBeta::DatabaseAwsRdsPostgres => {
                        let config: DbInput =
                            serde_json::from_value(klyra_resource.config.clone())
                        ...
                        let res = DatabaseResource::Info(
                            prov.provision_database(Request::new(DatabaseRequest {
                                ...
                                db_type: Some(
                                    database::Type::try_from(klyra_resource.r#type)
                        ...
                        ResourceResponseBeta {
                            ...
                            config: klyra_resource.config,
                            output: serde_json::to_value(res).unwrap(),
                        }
                    }
                    ResourceTypeBeta::Container => {
                        let config = serde_json::from_value(klyra_resource.config.clone())
                            .context("deserializing resource config")?;
