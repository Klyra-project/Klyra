...
use klyra_common::constants::RUNTIME_NAME;
use tokio::io::AsyncBufReadExt;
use tracing::{debug, error, info, trace};

#[derive(Clone, Debug, Eq, PartialEq)]
/// This represents a compiled Klyra service
...
fn extract_klyra_toml_name(path: PathBuf) -> anyhow::Result<String> {
    ...
    let name = toml
        .get("name")
        .context("couldn't find `name` key in Klyra.toml")?
        .as_str()
        .context("`name` key in Klyra.toml must be a string")?
        .to_string();
...
pub async fn build_workspace(
    ...
    if packages.is_empty() {
        bail!(
            "Did not find any packages that Klyra can run. \
            Make sure your crate has a binary target that uses `#[klyra_runtime::main]`."
        );
    }
...
pub fn find_klyra_packages(metadata: &Metadata) -> anyhow::Result<Vec<Package>> {
    let mut packages = Vec::new();
    for member in metadata.workspace_packages() {
        // skip non-Klyra-related crates
        ...
        let mut klyra_deps = member
            ...
            .collect::<Vec<_>>();
        klyra_deps.sort();
        info!(name = member.name, deps = ?klyra_deps, "Found workspace member with klyra dependencies");
...
async fn compile(
    ...
    for package in &packages {
        if package.features.contains_key("klyra") {
            cmd.arg("--no-default-features").arg("--features=klyra");
        }
    ...
    if !status.success() {
        bail!("Build failed. Is the Klyra runtime missing?");
    }
