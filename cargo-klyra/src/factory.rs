...
use klyra_common::{project::ProjectName, DatabaseReadyInfo};
use klyra_service::{error::CustomError, Factory};
...
impl Factory for LocalFactory {
    async fn get_sql_connection_string(&mut self) -> Result<String, klyra_service::Error> {
        ...
        let container_name = format!("klyra_{}_postgres", self.project);
...
impl LocalFactory {
    async fn wait_for_ready(&self, container_name: &str) -> Result<(), klyra_service::Error> {
        loop {
