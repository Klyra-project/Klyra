...
use klyra_service::{database::Type, error::CustomError, Environment, Factory, ServiceName};
use std::{
...
impl Factory for LocalFactory {
    async fn get_db_connection_string(
        ...
        let container_name = format!("klyra_{}_{}", self.service_name, r#type);
        ...
        let container = match self.docker.inspect_container(&container_name, None).await {
            ...
            Err(error) => {
                ...
                return Err(klyra_service::Error::Custom(CustomError::new(error)));
            }
        };
