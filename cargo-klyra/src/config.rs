...
use klyra_common::constants::API_URL_BETA;
use klyra_common::{constants::API_URL_DEFAULT, ApiKey};
use tracing::trace;
...
impl ConfigManager for GlobalConfigManager {
    fn directory(&self) -> PathBuf {
        let klyra_config_dir = dirs::config_dir()
            .ok_or_else(|| {
                anyhow!(
...
/// Klyra.toml schema (User-facing project-local config)
#[derive(Deserialize, Serialize, Default)]
pub struct ProjectConfig {
    // unused on new platform
    pub name: Option<String>,
    pub assets: Option<Vec<String>>,
    // unused in cargo-klyra, used in new platform builder.
    // is used here to validate the type if used.
    pub build_assets: Option<Vec<String>>,
}

/// .klyra/config.toml schema (internal project-local config)
#[derive(Deserialize, Serialize, Default)]
...
/// # Usage
/// ```rust,no_run
/// # use cargo_klyra::config::{Config, GlobalConfig, GlobalConfigManager};
/// #
...
impl RequestContext {
    ...
    pub fn load_local_internal(&mut self, project_args: &ProjectArgs) -> Result<()> {
        ...
        trace!(
            "looking for .klyra/config.toml in {}",
            workspace_path.display()
        );
        let local_manager =
            LocalConfigManager::new(workspace_path, ".klyra/config.toml".to_string());
        let mut project_internal = Config::new(local_manager);
        if !project_internal.exists() {
            trace!("no local .klyra/config.toml found");
            project_internal.replace(InternalProjectConfig::default());
        } else {
            trace!("found a local .klyra/config.toml");
        ...
        // 2. Name from .klyra/config.toml file
        match (&project_args.name_or_id, &config.id) {
            ...
            (None, Some(_)) => {
                trace!("using .klyra/config.toml project id");
            }
            (None, None) => {
    ...
    pub fn get_local_config(
        ...
        trace!("looking for Klyra.toml in {}", workspace_path.display());
        let local_manager = LocalConfigManager::new(workspace_path, "Klyra.toml".to_string());
        let mut project = Config::new(local_manager);
        if !project.exists() {
            trace!("no local Klyra.toml found");
            project.replace(ProjectConfig::default());
        } else {
            trace!("found a local Klyra.toml");
            project.open()?;
        }
        ...
        match (&project_args.name_or_id, &config.name) {
            ...
            // If key exists in config then keep it as it is
            (None, Some(_)) => {
                trace!("using Klyra.toml project name");
            }
    ...
    /// Get the API key from the `klyra_API_KEY` env variable, or
    /// otherwise from the global configuration. Returns an error if
    /// an API key is not set.
    pub fn api_key(&self) -> Result<ApiKey> {
        let api_key = std::env::var("klyra_API_KEY");

        if let Ok(key) = api_key {
            ApiKey::parse(&key).context("environment variable klyra_API_KEY is invalid")
        } else {
...
mod tests {
    ...
    #[test]
    fn get_local_config_finds_name_in_klyra_toml() {
