...
use klyra_common::constants::API_URL_DEFAULT;
use klyra_common::constants::API_URL_DEFAULT_BETA;
use tracing::trace;
...
impl RequestContext {
    ...
    pub fn api_key(&self) -> Result<String> {
        match std::env::var("klyra_API_KEY") {
            Ok(key) => Ok(key),
            Err(_) => match self.global.as_ref().unwrap().api_key() {
                ...
                .context(anyhow!(
                    "No valid API key found, try logging in first with `klyra login`"
                ))),
            },
        }
    }
