...
pub(crate) async fn auth(mut api_url: ApiUrl, username: String) -> Result<ApiKey> {
    ...
    let res: Response = client
        ...
        .send()
        .await
        .context("failed to get API key from Klyra server")?;
...
pub(crate) async fn delete(
    ...
    let res: Response = client
        ...
        .send()
        .await
        .context("failed to delete deployment on the Klyra server")?;
...
pub(crate) async fn klyra_version(mut api_url: ApiUrl) -> Result<String> {
    let client = get_retry_client();
    ...
    let res: Response = client
        ...
        .send()
        .await
        .context("failed to get version from Klyra server")?;
    ...
    if response_status == StatusCode::OK {
        ...
        error!(
            text = res.text().await?,
            status = %response_status,
            "failed to get klyra version from server"
        );
        Err(anyhow!("failed to get klyra version from server"))
    }
}
...
pub(crate) async fn deploy(
    ...
    let res: Response = client
        ...
        .header(klyra_PROJECT_HEADER, serde_json::to_string(&project)?)
        .basic_auth(api_key.clone(), Some(""))
        .send()
        .await
        .context("failed to send deployment to the Klyra server")?;
...
pub(crate) async fn secrets(
    ...
    client
        ...
        .send()
        .await
        .context("failed to send deployment's secrets to the Klyra server")
        .map(|_| ())
}
