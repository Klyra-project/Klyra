...
use regex::Regex;
use klyra_common::constants::klyra_EXAMPLES_README;
...
pub fn generate_project(
    ...
    // if the crate name was not updated, set it in Klyra.toml instead
    edit_klyra_toml(&path, (!crate_name_set).then_some(name))
        .context("Failed to edit Klyra.toml")?;
...
fn setup_template(auto_path: &str) -> Result<TempDir> {
    ...
    if let Some(caps) = git_re.captures(auto_path) {
        ...
        if url.scheme() == "http" || url.scheme() == "https" {
            ...
            println!(
                "HINT: You can find examples of how to select a template here: {klyra_EXAMPLES_README}"
            );
...
/// Remove or set the "name" field in Klyra.toml based on what is needed.
fn edit_klyra_toml(path: &Path, set_name: Option<&str>) -> Result<()> {
    let path = path.join("Klyra.toml");

    if set_name.is_none() && !path.exists() {
        // Do nothing if template has no Klyra.toml and the name should not be set
        return Ok(());
    }
...
fn create_ignore_file(path: &Path, name: &str) -> Result<()> {
    ...
    for rule in ["/target", ".klyra-storage", "Secrets*.toml"] {
...
mod tests {
    ...
    fn gix_clone_works() {
        let temp_dir = Builder::new()
            .prefix("klyra-clone-test")
            .tempdir()
            .unwrap();
        gix_clone(
            "https://github.com/klyra-hq/klyra-examples.git",
            temp_dir.path(),
        )
        .unwrap();
