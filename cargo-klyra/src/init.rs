...
pub fn get_framework(init_args: &InitArgs) -> Box<dyn KlyraInit> {
    ...
    if init_args.rocket {
        return Box::new(KlyraInitRocket);
    }

    if init_args.tide {
        return Box::new(KlyraInitTide);
    }
    ...
    if init_args.thruster {
        return Box::new(KlyraInitThruster);
    }
...
pub fn cargo_klyra_init(path: PathBuf, framework: Box<dyn KlyraInit>) -> Result<()> {
