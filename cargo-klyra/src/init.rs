...
const klyra_EXAMPLES_README: &str = "https://github.com/klyra\
		     -hq/klyra-examples#how-to-clone-run-and-deploy-an-example";
...
pub fn generate_project(dest: PathBuf, name: &str, temp_loc: TemplateLocation) -> Result<()> {
    ...
    edit_klyra_toml(&path).context("Failed to edit Klyra.toml")?;
...
fn setup_template(auto_path: &str) -> Result<TempDir> {
    ...
    if let Some(caps) = git_re.captures(auto_path) {
        ...
        if url.scheme() == "http" || url.scheme() == "https" {
		 ...
		 a template here: {klyra_EXAMPLES_README}"
            );
...
fn edit_klyra_toml(path: &Path) -> Result<()> {
    let path = path.join("Klyra.toml");
    if !path.exists() {
        // Do nothing if template has no Klyra.toml
        return Ok(());
    }
    let toml_str = read_to_string(&path)?;
    let mut doc = toml_str.parse::<Document>()?;

    // The Klyra.toml project name override will likely already be in use,
...
mod tests {
    ...
    fn gix_clone_works() {
        let temp_dir = Builder::new()
            ...
            .unwrap();
        gix_clone(
            "https://github.com/klyra-hq/awesome-klyra.git",
            temp_dir.path(),
        )
        .unwrap();
