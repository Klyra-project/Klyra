...
use klyra_common::{project::ProjectName, resource, ApiKey};
...
impl Klyra {
    ...
    pub async fn run(mut self, mut args: KlyraArgs) -> Result<CommandOutcome> {
        ...
        if !should_create_environment {
            ...
            if serenity_idle_hint {
                printdoc!(
                    ...
                    project so that they don't go offline: https://docs.klyra.rs/introduction/idle-projects
                    "
                );
            }
        }
    ...
    async fn local_run(&self, run_args: RunArgs) -> Result<()> {
        ...
        let (logger_server, logger_port) = Klyra::setup_local_logger().await?;
        ...
        for (i, service) in services.iter().enumerate() {
            ...
            signal_received = tokio::select! {
                res = Klyra::spin_local_runtime(&run_args, service, &provisioner_server, i as u16, provisioner_port, &logger_server, logger_port) => {
                    Klyra::add_runtime_info(res.unwrap(), &mut runtimes, &[&provisioner_server, &logger_server]).await?;
                    false
                },
    ...
    async fn local_run(&self, run_args: RunArgs) -> Result<()> {
        ...
        for (i, service) in services.iter().enumerate() {
            signal_received = tokio::select! {
                res = Klyra::spin_local_runtime(&run_args, service, &[&provisioner_server, &logger_server], i as u16, provisioner_port, logger_server,
                    logger_port,) => {
            ...
            if let Some(Ok(msg)) = message {
                if let tokio_tungstenite::tungstenite::Message::Text(line) = msg {
                    ...
                    match log_item.state.clone() {
                        klyra_common::deployment::State::Queued
                        | klyra_common::deployment::State::Building
                        | klyra_common::deployment::State::Built
                        | klyra_common::deployment::State::Loading => {
                            println!("{log_item}");
                        }
                        klyra_common::deployment::State::Crashed => {
                            ...
                            println!();
                            print!("cargo klyra logs {}", &deployment.id);
                            println!();
    ...
    async fn project_create(&self, client: &Client, idle_minutes: u64) -> Result<()> {
        ...
        if idle_minutes > 0 {
            ...
            println!("To change the idle time refer to the docs: https://docs.klyra.rs/introduction/idle-projects");
            println!();
        }
    ...
    fn make_archive(&self) -> Result<Vec<u8>> {
        ...
        if secrets_path.exists() {
            entries.insert(secrets_path, Path::new("klyra").join("Secrets.toml"));
        }
...
mod tests {
    ...
    fn get_archive_entries(mut project_args: ProjectArgs) -> Vec<String> {
        ...
        klyra.load_project(&mut project_args).unwrap();
