...
use klyra_common::models::deployment::{deployments_table_beta, DeploymentRequestBeta};
use klyra_common::{
    constants::{
...
use klyra_service::builder::{async_cargo_metadata, find_klyra_packages};
use klyra_service::{
...
impl Klyra {
    ...
    pub async fn run(
        ...
        if matches!(
            ...
            let client = KlyraApiClient::new(self.ctx.api_url(), self.ctx.api_key().ok());
    ...
    async fn logs_beta(&self, args: LogsArgs) -> Result<CommandOutcome> {
        ...
        while let Some(Ok(msg)) = stream.next().await {
            if let tokio_tungstenite::tungstenite::Message::Text(line) = msg {
                match serde_json::from_str::<klyra_common::LogItemBeta>(&line) {
                    Ok(log) => {
                        if args.raw {
                            println!("{}", log.line);
                        } else {
