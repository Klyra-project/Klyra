...
use klyra_common::{
    constants::{
        headers::X_CARGO_klyra_VERSION, API_URL_DEFAULT, API_URL_DEFAULT_BETA,
        DEFAULT_IDLE_MINUTES, EXAMPLES_REPO, EXECUTABLE_DIRNAME, RESOURCE_SCHEMA_VERSION,
        RUNTIME_NAME, klyra_IDLE_DOCS_URL, klyra_LEGACY_NEW_PROJECT, STORAGE_DIRNAME,
...
use klyra_service::{
    builder::{async_cargo_metadata, build_workspace, find_klyra_packages, BuiltService},
    runner, Environment,
};
...
pub use crate::args::{Command, ProjectArgs, RunArgs, KlyraArgs};
...
use crate::util::{
    ...
    is_dirty, open_gh_issue, read_ws_until_text, update_cargo_klyra,
};
...
impl Binary {
    pub fn name(&self) -> String {
        match self {
            Self::CargoKlyra => "cargo-klyra".to_owned(),
            Self::Klyra => "klyra".to_owned(),
        }
    }
}
...
impl Klyra {
    ...
    pub async fn run(mut self, mut args: KlyraArgs, provided_path_to_init: bool) -> Result<()> {
        if self.bin == Binary::Klyra {
            // beta is always enabled in `klyra`
            args.beta = true;
        }
        self.beta = args.beta;
        if self.beta {
            if matches!(args.cmd, Command::Project(ProjectCommand::Restart { .. })) {
                bail!("This command is discontinued on the NEW platform (klyra.dev). Deploy to start a new deployment.");
            }
            if matches!(args.cmd, Command::Status) {
                bail!("This command is discontinued on the NEW platform (klyra.dev). Use `deployment status` instead.");
            }
            if matches!(
                args.cmd,
                Command::Stop | Command::Project(ProjectCommand::Stop { .. })
            ) {
                bail!("This command is discontinued on the NEW platform (klyra.dev). Use `deployment stop` instead.");
            }
            if matches!(args.cmd, Command::Clean) {
                bail!("This command is discontinued on the NEW platform (klyra.dev).");
            }
            if matches!(args.cmd, Command::Resource(ResourceCommand::Dump { .. })) {
                bail!("This command is not yet supported on the NEW platform (klyra.dev).");
            }
        } else if matches!(
            args.cmd,
        ...
        let res = match args.cmd {
            ...
            Command::Upgrade { preview } => update_cargo_klyra(preview).await,
        };
    ...
    async fn check_api_versions(&mut self) -> Result<()> {
        ...
        if let Ok(versions) = client.get_api_versions().await {
            ...
            // check cargo-klyra version
            // should always be a valid semver
            let my_version = &semver::Version::from_str(VERSION).unwrap();
            let latest_version = &self.version_info.as_ref().unwrap().cargo_klyra;
            if my_version != latest_version {
                ...
                let string = if semvers_are_compatible(my_version, latest_version) {
                    newer_version_exists.then(|| {
                        format!("Info: A newer version of cargo-klyra exists ({latest_version}).")
                    })
                    ...
                    newer_version_exists.then(||
                        formatdoc! {"
                            Warning:
                                A newer version of cargo-klyra exists ({latest_version}).
                                It is recommended to upgrade.
                                Refer to the upgrading docs: https://docs.klyra.rs/configuration/klyra-versions#upgrading-klyra-version"
                        }
                    ).or_else(||
                        Some(formatdoc! {"
                            Warning:
                                Your version of cargo-klyra ({my_version}) is newer than what the API expects ({latest_version}).
    ...
    /// Log in, initialize a project and potentially create the Klyra environment for it.
    ///
    ...
    async fn init(
        ...
        if needs_name && !self.beta {
            printdoc! {"
                ...
                It will be hosted at ${{project_name}}.klyraapp.rs, so choose something unique!
                "
            };
        }
        ...
        let template = match git_template {
            ...
            None => {
                ...
                let schema = if offline {
                    ...
                    get_templates_schema()
                        ...
                        .and_then(|s| {
                            ...
                            println!(
                                "{}",
                                "Template list with incompatible version found. Consider updating cargo-klyra. Falling back to internal list."
                                    .yellow()
                            );
                ...
                if let Some(schema) = schema {
                    ...
                    println!("Klyra works with many frameworks. Which one do you want to use?");
        ...
        // 6. Confirm that the user wants to create the project environment on Klyra
        let should_create_environment = if !interactive {
            ...
            let should_create = Confirm::with_theme(&theme)
                .with_prompt(if self.beta {
                    format!(r#"Create a project on Klyra with the name "{name}"?"#)
                } else {
                    format!(
                        r#"Claim the project name "{name}" by starting a project container on Klyra?"#
                    )
                })
                .default(true)
                .interact()?;
            if !should_create && !self.beta {
                println!(
                    ...
                    you start the project with `cargo klyra project start`."
                )
            }
            println!();
            should_create
        ...
        if self.beta {
            println!("Run `klyra run` to run the app locally.");
        } else {
            println!("Run `cargo klyra run` to run the app locally.");
        }
        if !should_create_environment {
            if self.beta {
                println!("Run `klyra deploy` to deploy it to Klyra.");
            } else {
                println!(
                ...
                if serenity_idle_hint {
                    printdoc!(
                        ...
                        project so that they don't go offline: {klyra_IDLE_DOCS_URL}
                        "
                    );
                }
            }
        ...
        let api_key = match login_args.api_key {
            ...
            None => {
                if login_args.prompt || !self.beta {
                    // manual input requested (always the case on klyra.rs)

                    if !login_args.prompt && !self.beta {
                        // if !beta, open console
                        let url = klyra_LEGACY_NEW_PROJECT;
                    ...
                    // device auth flow via Klyra Console
    ...
    async fn device_auth(&self, console_url: String) -> Result<String> {
        ...
        let _ = webbrowser::open(url);
        println!("Complete login in Klyra Console to authenticate CLI.");
        println!("If your browser did not automatically open, go to {url}");
        println!();
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        if logout_args.reset_api_key {
            ...
            if self.beta {
                println!(" -> Use `klyra login` to get a new one.")
            } else {
                println!(" -> Go to {klyra_LEGACY_NEW_PROJECT} to get a new one.");
            }
            println!();
        }
    ...
    async fn stop(&self) -> Result<()> {
        ...
        wait_with_spinner(500, |i, pb| async move {
            ...
            let cleanup = move || {
                ...
                println!("{service_str}");
                println!("Run `cargo klyra deploy` to re-deploy your service.");
            };
            ...
            if deployment.state == klyra_common::deployment::State::Stopped {
                Ok(Some(cleanup))
            } else {
                Ok(None)
            }
    ...
    async fn resource_delete(
        ...
        println!(
            ...
            formatdoc! {"
                Note:
                    Remember to remove the resource annotation from your #[klyra_runtime::main] function.
    ...
    async fn spin_local_runtime(
        ...
        let secrets = Klyra::get_secrets(run_args, service)?;
        ...
        match runtime {
            ...
            None => {
                ...
                for rt_info in existing_runtimes {
                    ...
                    Klyra::stop_runtime(&mut rt_info.0, &mut rt_info.1)
                        .await
                        .unwrap_or_else(|_| {
                            errored_out = true;
                        });
    ...
    async fn local_run(&self, mut run_args: RunArgs) -> Result<()> {
        ...
        Klyra::find_available_port(&mut run_args, services.len());
    ...
    async fn local_run_beta(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        let service = services
            .first()
            .expect("at least one klyra service")
            .to_owned();

        trace!(path = ?service.executable_path, "runtime executable");

        let secrets = Klyra::get_secrets(&run_args, &service)?;
        Klyra::find_available_port_beta(&mut run_args);
        if let Some(warning) = check_and_warn_runtime_version(&service.executable_path).await? {
            eprint!("{}", warning);
        }
        ...
        let mut envs = vec![
            ("klyra_BETA", "true".to_owned()),
            ("klyra_PROJECT_ID", "proj_LOCAL".to_owned()),
            ("klyra_PROJECT_NAME", project_name),
            ("klyra_ENV", Environment::Local.to_string()),
            ("klyra_RUNTIME_IP", ip.to_string()),
            ("klyra_RUNTIME_PORT", run_args.port.to_string()),
            ("klyra_API", format!("http://127.0.0.1:{}", api_port)),
        ];
        // Use a nice debugging tracing level if user does not provide their own
        if debug && std::env::var("RUST_LOG").is_err() {
            envs.push(("RUST_LOG", "info,klyra=trace,reqwest=debug".to_owned()));
        }
        ...
        let exit_result = {
            ...
            tokio::select! {
                ...
                _ = sigterm_notif.recv() => {
                    eprintln!("cargo-klyra received SIGTERM. Killing the runtime...");
                    None
                },
                _ = sigint_notif.recv() => {
                    eprintln!("cargo-klyra received SIGINT. Killing the runtime...");
                    None
                }
            }
        };
        ...
        let exit_result = {
            ...
            tokio::select! {
                ...
                _ = ctrl_break_notif.recv() => {
                    eprintln!("cargo-klyra received ctrl-break.");
                    None
                },
                _ = ctrl_c_notif.recv() => {
                    eprintln!("cargo-klyra received ctrl-c.");
                    None
                },
                _ = ctrl_close_notif.recv() => {
                    eprintln!("cargo-klyra received ctrl-close.");
                    None
                },
                _ = ctrl_logoff_notif.recv() => {
                    eprintln!("cargo-klyra received ctrl-logoff.");
                    None
                },
                _ = ctrl_shutdown_notif.recv() => {
                    eprintln!("cargo-klyra received ctrl-shutdown.");
                    None
                }
            }
        };
    ...
    async fn handle_signals() -> bool {
        ...
        tokio::select! {
            _ = ctrl_break_notif.recv() => {
                println!("cargo-klyra received ctrl-break.");
                true
            },
            _ = ctrl_c_notif.recv() => {
                println!("cargo-klyra received ctrl-c.");
                true
            },
            _ = ctrl_close_notif.recv() => {
                println!("cargo-klyra received ctrl-close.");
                true
            },
            _ = ctrl_logoff_notif.recv() => {
                println!("cargo-klyra received ctrl-logoff.");
                true
            },
            _ = ctrl_shutdown_notif.recv() => {
                println!("cargo-klyra received ctrl-shutdown.");
                true
            }
            else => {
                false
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        let secrets = if self.beta {
            Klyra::get_secrets_beta(&args.secret_args, working_directory)?
        } else {
            None
        };
        ...
        if !self.beta && archive.len() > CREATE_SERVICE_BODY_LIMIT {
            bail!(
                ...
                Run with `cargo klyra --debug` to see which files are being packed."#,
        ...
        while let Ok(Some(s)) = read_ws_until_text(&mut stream).await {
                ...
                && log_item.line.contains("Deployer version: ")
            {
                ...
                if latest_version > my_version {
                    self.version_warnings.push(
                                formatdoc! {"
                                    Warning:
                                        A newer version of klyra-deployer is available ({latest_version}).
                                        Use `cargo klyra project restart` to upgrade."
                                }
                                .yellow()
                                .to_string(),
                            )
            ...
            if !runtime_version_checked
                ...
                && log_item
                    .line
                    .contains("klyra-runtime executable started (version ")
            {
                runtime_version_checked = true;
                let my_version = &log_item
                    .line
                    .split_once("klyra-runtime executable started (version ")
                    .unwrap()
                    .1
                    .split_once(')')
                    .unwrap()
                ...
                if latest_version > my_version {
                    self.version_warnings.push(
                                formatdoc! {"
                                    Warning:
                                        A newer version of klyra-runtime is available ({latest_version}).
                                        Update it and any other klyra dependencies in Cargo.toml."
                                }
                                .yellow()
                                .to_string(),
                            )
                ...
                .any(|m| log_item.line.contains(m))
            {
                ...
                println!();
                println!("cargo klyra logs {}", &deployment.id);
        ...
        if deployment.state != klyra_common::deployment::State::Running {
            ...
            match deployment.state {
                ...
                klyra_common::deployment::State::Crashed => {
                    println!(
                        "{}",
    ...
    async fn project_start(&self, idle_minutes: u64) -> Result<()> {
        ...
        if idle_minutes > 0 && !self.beta {
            ...
            println!("To change the idle time refer to the docs: {klyra_IDLE_DOCS_URL}");
            println!();
        }
    ...
    async fn project_stop(&self) -> Result<()> {
        ...
        println!("Run `cargo klyra project start` to recreate project environment on Klyra.");
            ...
            println!(
                ...
                formatdoc!(
                    ...
                    WARNING:
                        ...
                        This will...
                        - Delete any databases, secrets, and klyra-persist data in this project.
                        - Delete any custom domains linked to this project.
...
mod tests {
    ...
    async fn get_archive_entries(
        ...
        let mut klyra = Klyra::new(crate::Binary::CargoKlyra).unwrap();
        klyra
    ...
    async fn make_archive_respect_rules() {
        let working_directory = canonicalize(path_from_workspace_root(
            "cargo-klyra/tests/resources/archiving",
        ))
        .unwrap();
        ...
        let expected = vec![
            ...
            "Klyra.toml",
            "asset1", // normal file
            "asset2", // .gitignore'd, but included in Klyra.toml
            // asset3 is .ignore'd
            "asset4",                // .gitignore'd, but un-ignored in .ignore
            "asset5",                // .ignore'd, but included in Klyra.toml
            "dist/dist1",            // .gitignore'd, but included in Klyra.toml
