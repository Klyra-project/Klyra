...
use klyra_common::models::deployment::get_deployments_table;
...
impl Klyra {
    ...
    async fn init(&mut self, args: InitArgs, mut project_args: ProjectArgs) -> Result<()> {
        ...
        if project_args.name.is_none() {
            println!("How do you want to name your project? It will be hosted at ${{project_name}}.klyraapp.rs.");
        ...
        init::cargo_klyra_init(path.clone(), framework)?;
        println!();
