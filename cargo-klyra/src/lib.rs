...
use klyra_common::{
    constants::{
        headers::X_CARGO_klyra_VERSION, API_URL_BETA, API_URL_DEFAULT, DEFAULT_IDLE_MINUTES,
        EXAMPLES_REPO, EXECUTABLE_DIRNAME, RESOURCE_SCHEMA_VERSION, RUNTIME_NAME,
        klyra_GH_ISSUE_URL, klyra_GH_REPO_URL, klyra_IDLE_DOCS_URL, klyra_INSTALL_DOCS_URL,
        klyra_LOGIN_URL, klyra_LOGIN_URL_BETA, STORAGE_DIRNAME, TEMPLATES_SCHEMA_VERSION,
    },
...
impl Klyra {
    ...
    async fn init(
        ...
        let should_create_environment = if self.beta {
            ...
            let should_create = Confirm::with_theme(&theme)
                .with_prompt(format!(
                    r#"Claim the project name "{}" by starting a project container on Klyra?"#,
                    project_args
                        .name_or_id
                        .as_ref()
                        .expect("to have a project name provided")
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        if logout_args.reset_api_key {
            ...
            println!(" -> Go to {klyra_LOGIN_URL} to get a new one.\n");
        }
                ...
                if mismatch.klyra_runtime > mismatch.cargo_klyra {
                    ...
                    printdoc! {"
                        Hint: A newer version of cargo-klyra is available.
                              Check out the installation docs for how to update: {klyra_INSTALL_DOCS_URL}",
                    };
                } else {
                    printdoc! {"
    ...
    async fn spin_local_runtime(
        ...
        Klyra::check_and_warn_runtime_version(&service.executable_path).await?;
    ...
    async fn local_run_beta(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        .envs([
            ("klyra_BETA", "true"),
            ("klyra_PROJECT_ID", "proj_LOCAL"),
            ("klyra_PROJECT_NAME", project_name.as_str()),
            ("klyra_ENV", Environment::Local.to_string().as_str()),
            ("klyra_RUNTIME_IP", ip.to_string().as_str()),
            ("klyra_RUNTIME_PORT", run_args.port.to_string().as_str()),
            (
                "klyra_API",
            ...
            (
                "RUST_LOG",
                if debug {
                    "info,klyra=trace,reqwest=debug"
                } else {
                    "info"
                },
            ),
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        loop {
            if let Some(Ok(msg)) = stream.next().await {
                if let tokio_tungstenite::tungstenite::Message::Text(line) = msg {
                    let log_item = match serde_json::from_str::<klyra_common::LogItem>(&line) {
...
async fn update_cargo_klyra(preview: bool) -> Result<()> {
    if preview {
    ...
    let _ = tokio::process::Command::new("bash")
        .args(["-c", "curl -sSfL https://www.klyra.rs/install | bash"])
        .kill_on_drop(true)
        .spawn()
    ...
    let _ = tokio::process::Command::new("powershell")
        .args(["-Command", "iwr https://www.klyra.rs/install-win | iex"])
        .kill_on_drop(true)
        .spawn()
