...
impl Klyra {
    ...
    pub async fn parse_args_and_run(self) -> Result<CommandOutcome> {
        ...
        let args = KlyraArgs::from_arg_matches(&matches)
    ...
    async fn init(
        ...
        let template = match git_templates {
            ...
            None => {
                println!(
                    "Klyra works with a range of web frameworks. Which one do you want to use?"
                );
        ...
        printdoc!(
            ...
            Hint: Check the examples repo for a full list of templates:
                  {EXAMPLES_REPO}
            Hint: You can also use `cargo klyra init --from` to clone templates.
                  See {klyra_CLI_DOCS_URL}
                  or run `cargo klyra init --help`
            "
        );
        println!();
    ...
    async fn login(&mut self, login_args: LoginArgs) -> Result<CommandOutcome> {
        let api_key_str = match login_args.api_key {
            ...
            None => {
                ...
                println!("If your browser did not automatically open, go to {klyra_LOGIN_URL}");
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<CommandOutcome> {
        ...
        println!("Successfully logged out of klyra.");
    ...
    > {
        ...
        let runtime_executable = if service.is_wasm {
            ...
            println!("Installing klyra-next runtime. This can take a while...");
            ...
            if let Err(err) = check_version(&service.executable_path) {
                ...
                if let Some(mismatch) = err.downcast_ref::<VersionMismatchError>() {
                    ...
                    return Err(err.context(
                        format!(
                            "Failed to verify the version of klyra-runtime in {}. Is cargo targeting the correct binary?",
    ...
    async fn pre_local_run(&self, run_args: &RunArgs) -> Result<Vec<BuiltService>> {
        ...
        // Compile all the alpha or klyra-next services in the workspace.
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<CommandOutcome> {
        ...
        if deployment_req.data.len() > CREATE_SERVICE_BODY_LIMIT {
            bail!(
                ...
                Run with `RUST_LOG="cargo_klyra=debug"` to see which files are being packed."#,
        ...
        client.delete_project(self.ctx.project_name(), true).await.map_err(|err| {
            if let Some(api_error) = err.downcast_ref::<ApiError>() {
                ...
                if api_error.message.contains("not ready") {
                    ...
                    println!();
                    println!("{}", "Try restarting the project with `cargo klyra project restart` first.".yellow());
                    println!("{}", "This is needed to check for any resources linked to it.".yellow());
                    println!("{}", "For more help with deleting projects, visit https://docs.klyra.rs/support/delete-project".yellow());
                    println!();
                    return err;
                }
            }
        ...
        println!(
            ...
            formatdoc!(
                ...
                WARNING:
                    ...
                    This will...
                    - Delete any klyra-persist data in this project.
...
mod tests {
    ...
    fn get_archive_entries(project_args: ProjectArgs) -> Vec<String> {
        ...
        let archive = klyra.make_archive().unwrap();
