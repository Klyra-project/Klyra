...
impl Klyra {
    ...
    async fn init(&mut self, args: InitArgs, mut project_args: ProjectArgs) -> Result<()> {
        ...
        init::cargo_klyra_init(path, framework)?;
        println!();
...
{}"#,
    ...
    fn package_secret(&self, file: File) -> Result<Vec<u8>> {
        ...
        if secrets_path.exists() {
            archive_write
                .append_path_with_name(secrets_path, Path::new("klyra").join("Secrets.toml"))?;
        }
...
mod tests {
    ...
    fn secrets_file_is_archived() {
        ...
        let file = klyra.run_cargo_package(true).unwrap();
        ...
        let file = klyra.run_cargo_package(true).unwrap();
        let new_file = klyra.package_secret(file).unwrap();
