...
use klyra_common::{
    constants::{
        headers::X_CARGO_klyra_VERSION, API_URL_DEFAULT_BETA, EXAMPLES_REPO, RUNTIME_NAME,
...
impl Klyra {
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        ...
        println!(" -> Use `klyra login` to log in again.");
    ...
    async fn local_run_beta(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        let secrets = Klyra::get_secrets_beta(&run_args.secret_args, working_directory)?
            .unwrap_or_default();
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        let secrets = Klyra::get_secrets_beta(&args.secret_args, working_directory)?;
