...
use klyra_common::{
    constants::{
        ...
        klyra_LOGIN_URL, STORAGE_DIRNAME, TEMPLATES_SCHEMA_VERSION,
    },
...
impl Klyra {
    ...
    pub async fn run(mut self, mut args: KlyraArgs, provided_path_to_init: bool) -> Result<()> {
        ...
        if self.beta {
            ...
            bail!("This command is not supported on the legacy platform. Set --beta or klyra_BETA=true.");
        }
    ...
    async fn init(
        ...
        if !should_create_environment {
            if self.beta {
                println!("Run `cargo klyra deploy --allow-dirty` to deploy it to Klyra.");
            } else {
                println!(
    ...
    fn feedback(&self) -> Result<()> {
        ...
        println!("If your browser did not open automatically, go to {klyra_GH_ISSUE_URL}");
    ...
    async fn login(&mut self, login_args: LoginArgs, offline: bool) -> Result<()> {
        let api_key_str = match login_args.api_key {
            ...
            None => {
                if !offline {
                    let _ = webbrowser::open(klyra_LOGIN_URL);
                    println!(
                        "If your browser did not automatically open, go to {klyra_LOGIN_URL}"
                    );
                }
...
mod tests {
    ...
    fn get_archive_entries(
        ...
        klyra.load_project(&project_args).unwrap();
