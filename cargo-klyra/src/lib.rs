...
use klyra_builder::render_rust_dockerfile;
use klyra_common::{
    constants::{
...
use crate::builder::{async_cargo_metadata, build_workspace, find_klyra_packages, BuiltService};
...
use crate::util::{
    ...
    get_templates_schema, is_dirty, open_gh_issue, read_ws_until_text, update_cargo_klyra,
};
...
impl Klyra {
    ...
    pub async fn run(mut self, args: KlyraArgs, provided_path_to_init: bool) -> Result<()> {
    ...
    async fn init(
        ...
        if needs_login {
            eprintln!("First, let's log in to your Klyra account.");
        ...
        let template = match git_template {
            ...
            None => {
                ...
                let schema = if offline {
                    ...
                    get_templates_schema()
                        ...
                        .and_then(|s| {
                            ...
                            eprintln!(
                                "{}",
                                "Template list with incompatible version found. Consider upgrading Klyra CLI. Falling back to internal list."
                                    .yellow()
                            );
                ...
                if let Some(schema) = schema {
                    ...
                    eprintln!("Klyra works with many frameworks. Which one do you want to use?");
        ...
        let should_create_environment = if !interactive {
            ...
            let should_create = Confirm::with_theme(&theme)
                .with_prompt(format!(
                    r#"Create a project on Klyra with the name "{name}"?"#
                ))
                .default(true)
                .interact()?;
            eprintln!();
        ...
        eprintln!("Run `klyra run` to run the app locally.");
        if !should_create_environment {
            eprintln!("Run `klyra deploy` to deploy it to Klyra.");
        }
    ...
    async fn device_auth(&self, console_url: Option<String>) -> Result<String> {
        ...
        let url = token_message.url.unwrap_or_else(|| {
            format!(
                "{}/device-auth?token={}",
                console_url.as_deref().unwrap_or(klyra_CONSOLE_URL),
                token
            )
        });
    ...
    async fn logout(&mut self, logout_args: LogoutArgs) -> Result<()> {
        ...
        eprintln!(" -> Use `klyra login` to log in again.");
    ...
    async fn logs(&self, args: LogsArgs) -> Result<()> {
        if args.follow {
            eprintln!("Streamed logs are not yet supported on the klyra.dev platform.");
            return Ok(());
        }
        if args.tail.is_some() | args.head.is_some() {
            eprintln!("Fetching log ranges are not yet supported on the klyra.dev platform.");
            return Ok(());
        }
        let client = self.client.as_ref().unwrap();
        let pid = self.ctx.project_id();
    ...
    async fn local_run(&self, mut run_args: RunArgs, debug: bool) -> Result<()> {
        ...
        let mut envs = vec![
            ...
            ("klyra_HEALTHZ_PORT", healthz_port.to_string()),
    ...
    async fn local_docker_build(&self) -> Result<()> {
        ...
        fs::write(tempdir.join("klyra_prebuild.sh"), "")?;
        ...
        // TODO?: Support custom klyra.Dockerfile
        let dockerfile = tempdir.join("__klyra.Dockerfile");
        ...
        let docker = tokio::process::Command::new("docker")
            ...
            .arg(dockerfile)
            .arg("--tag")
            .arg(format!("klyra-build-{project_name}"))
            .arg(tempdir)
            .kill_on_drop(true)
            .spawn();
