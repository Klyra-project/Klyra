...
async fn cargo_klyra_init(path: PathBuf) -> anyhow::Result<CommandOutcome> {
...
/// creates a `cargo-klyra` init instance for initializing the `rocket` framework
async fn cargo_klyra_init_framework(path: PathBuf) -> anyhow::Result<CommandOutcome> {
    let working_directory = Path::new(".").to_path_buf();
...
async fn basic_init() {
    ...
    cargo_klyra_init(temp_dir_path.clone()).await.unwrap();
...
async fn framework_init() {
    ...
    cargo_klyra_init_framework(temp_dir_path.clone())
        .await
        .unwrap();
