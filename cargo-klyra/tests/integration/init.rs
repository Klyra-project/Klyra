...
use cargo_klyra::{Klyra, KlyraArgs};
use clap::Parser;
use indoc::indoc;
...
async fn non_interactive_basic_init() {
    ...
    Klyra::new().unwrap().run(args, true).await.unwrap();
...
async fn non_interactive_init_with_from_url() {
    ...
    let args = KlyraArgs::parse_from([
        ...
        "my-project",
        "--from",
        "https://github.com/klyra-hq/klyra-examples",
        "--subfolder",
        "tower/hello-world",
        temp_dir_path.to_str().unwrap(),
    ]);
    ...
    let cargo_toml = read_to_string(temp_dir_path.join("Cargo.toml")).unwrap();
    assert!(cargo_toml.contains("name = \"my-project\""));
    assert!(cargo_toml.contains("klyra-tower ="));
...
async fn non_interactive_init_with_from_local_path() {
    ...
    let args = KlyraArgs::parse_from([
        ...
        "my-project",
        "--from",
        "../examples", // cargo runs the test from the cargo-klyra folder.
        "--subfolder",
        "tower/hello-world",
        temp_dir_path.to_str().unwrap(),
    ]);
...
async fn non_interactive_init_from_local_path_with_workspace() {
    ...
    let klyra_toml = read_to_string(temp_dir_path.join("Klyra.toml")).unwrap();
    assert!(klyra_toml.contains("name = \"my-project\""));
...
fn interactive_rocket_init() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bin_path = assert_cmd::cargo::cargo_bin("cargo-klyra");
    ...
    session.exp_string("container on Klyra?")?;
    session.send("n")?;
    session.flush()?;
...
fn assert_valid_rocket_project(path: &Path, name: &str) {
    ...
    assert!(cargo_toml.contains("klyra-runtime = "));
    assert!(cargo_toml.contains("klyra-rocket = "));
