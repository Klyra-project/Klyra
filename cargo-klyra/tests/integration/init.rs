...
async fn non_interactive_init_with_from_local_path() {
    ...
    let args = KlyraArgs::parse_from([
        ...
        "my-project",
        "--from",
        "../examples", // for some reason, cargo runs the test from the cargo-klyra folder.
        "--subfolder",
        "tower/hello-world",
        temp_dir_path.to_str().unwrap(),
    ]);
...
fn interactive_rocket_init() -> Result<(), Box<dyn std::error::Error>> {
    ...
    session.exp_string("Do you want to create the project environment on Klyra?")?;
    session.send("n")?;
    session.flush()?;
