use cargo_klyra::{Command, ProjectArgs, RunArgs, Klyra, KlyraArgs};
...
async fn cargo_klyra_run(working_directory: &str, external: bool) -> String {
    ...
    let runner = Klyra::new().unwrap().run(KlyraArgs {
...
async fn rocket_hello_world() {
    let url = cargo_klyra_run("../examples/rocket/hello-world", false).await;
...
async fn rocket_secrets() {
    let url = cargo_klyra_run("../examples/rocket/secrets", false).await;
...
async fn rocket_postgres() {
    let url = cargo_klyra_run("../examples/rocket/postgres", false).await;
    let client = reqwest::Client::new();
...
async fn axum_static_files() {
    let url = cargo_klyra_run("../examples/axum/static-files", false).await;
    let client = reqwest::Client::new();
    ...
    assert!(
        request_text.contains("This is an example of serving static files with axum and klyra.")
    );
}
...
#[tokio::test(flavor = "multi_thread")]
async fn klyra_next() {
    let url = cargo_klyra_run("../examples/next/hello-world", false).await;
    let client = reqwest::Client::new();
...
async fn rocket_authentication() {
    let url = cargo_klyra_run("../examples/rocket/authentication", false).await;
    let client = reqwest::Client::new();
...
async fn actix_web_hello_world() {
    let url = cargo_klyra_run("../examples/actix-web/hello-world", false).await;
...
async fn axum_hello_world() {
    let url = cargo_klyra_run("../examples/axum/hello-world", false).await;
...
async fn tide_hello_world() {
    let url = cargo_klyra_run("../examples/tide/hello-world", false).await;
...
async fn tower_hello_world() {
    let url = cargo_klyra_run("../examples/tower/hello-world", false).await;
...
async fn warp_hello_world() {
    let url = cargo_klyra_run("../examples/warp/hello-world", false).await;
...
async fn poem_hello_world() {
    let url = cargo_klyra_run("../examples/poem/hello-world", false).await;
...
async fn poem_mongodb() {
    ...
    assert_eq!(request_text, "{\"note\":\"Deploy to klyra\"}");
}

#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn salvo_hello_world() {
    let url = cargo_klyra_run("../examples/salvo/hello-world", false).await;
...
async fn thruster_hello_world() {
    let url = cargo_klyra_run("../examples/thruster/hello-world", false).await;
...
async fn rocket_hello_world_with_router_ip() {
    let url = cargo_klyra_run("../examples/rocket/hello-world", true).await;
