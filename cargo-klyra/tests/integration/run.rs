...
/// Runs `klyra run` in specified directory
async fn klyra_run(working_directory: &str, external: bool) -> String {
    ...
    let runner = Klyra::new(cargo_klyra::Binary::Klyra, None)
        .unwrap()
        .run(
            KlyraArgs {
...
async fn axum_hello_world() {
    let url = klyra_run("../examples/axum/hello-world", false).await;
...
async fn rocket_secrets() {
    ...
    let url = klyra_run("../examples/rocket/secrets", false).await;
...
mod needs_docker {
    ...
    async fn rocket_postgres() {
        let url = klyra_run("../examples/rocket/postgres", false).await;
        let client = reqwest::Client::new();
