...
/// creates a `cargo-klyra` Command instance with some reasonable defaults set.
fn cargo_klyra_command() -> Command {
    let mut cmd = Command::cargo_bin("cargo-klyra").unwrap();
    cmd.env(
        "klyra_API",
...
fn default_prints_usage_to_stderr() {
    let mut cmd = cargo_klyra_command();
    cmd.assert()
        .stderr(predicates::str::is_match("^cargo-klyra.*\n\nUSAGE:").unwrap());
}
...
fn help_prints_usage_to_stdout() {
    ...
    cmd.arg("--help")
        .assert()
        .stdout(predicates::str::is_match("^cargo-klyra.*\n\nUSAGE:").unwrap());
}
