...
$ docker volume create klyra-cargo-registry-vol
```

Finally, you can start a local deployment of klyra and the required containers with:
...
First, `provisioner`, `deployer`, and `cargo-klyra` are binary crates with `provisioner` and `deployer` being backend services. The `cargo-klyra` binary is the `cargo klyra` command used by users.
