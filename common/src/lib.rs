...
pub const klyra_PROJECT_HEADER: &str = "Klyra-Project";

#[cfg(debug_assertions)]
lazy_static! {
    pub static ref API_URL: String =
        std::env::var("klyra_API").unwrap_or_else(|_| "http://localhost:8001".to_string());
}

#[cfg(not(debug_assertions))]
lazy_static! {
    pub static ref API_URL: String = "https://api.klyra.rs".to_string();
}
...
impl DeploymentMeta {
    ...
    pub fn create_host(project_config: &ProjectConfig) -> Host {
        format!("{}.klyraapp.rs", project_config.name())
    }
}
...
#[cfg(not(debug_assertions))]
const PUBLIC_IP: &'static str = "pg.klyra.rs";
