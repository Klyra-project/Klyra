...
pub fn get_resource_tables(
    ...
    let resource_groups = resources.iter().fold(HashMap::new(), |mut acc, x| {
        let title = match x.r#type {
            ...
            // ignore variants that don't represent Klyra-hosted resources
...
fn get_databases_table(
    ...
    for database in databases {
        ...
        let conn_string = match info {
            ...
            DatabaseResource::Info(info) => {
                if info.hostname_klyra == "localhost" && info.hostname_public == "localhost" {
    ...
    let show_secret_hint = if databases.is_empty() || show_secrets {
        ...
        "Hint: you can show the secrets of these resources using `cargo klyra resource list --show-secrets`\n"
    };
...
fn get_databases_table_beta(
    ...
    let show_secret_hint = if databases.is_empty() || show_secrets {
        ...
        "Hint: you can show the secrets of these resources using `klyra resource list --show-secrets`\n"
    };
