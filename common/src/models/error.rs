    ...
    fn from(kind: ErrorKind) -> Self {
        let (status, error_message) = match kind {
            ...
            ErrorKind::ProjectNotFound(project_name) => {
                return Self {
                    message: format!("Project '{}' not found. Make sure you are the owner of this project name. Run `cargo klyra project start` to create a new project.", project_name),
                    status_code: StatusCode::NOT_FOUND.as_u16(),
                }
            },
            ErrorKind::ProjectNotReady => (
                StatusCode::SERVICE_UNAVAILABLE,
                // "not ready" is matched against in cargo-klyra for giving further instructions on project deletion
                "Project not ready. Try running `cargo klyra project restart`.",
            ),
            ...
            ErrorKind::ProjectHasRunningDeployment => (
                ...
                "Could not automatically stop the running deployment for the project. Please reach out to Klyra support for help."
            ),
            ErrorKind::ProjectHasBuildingDeployment => (
                StatusCode::BAD_REQUEST,
                "Project currently has a deployment that is busy building. Use `cargo klyra deployment list` to see it and wait for it to finish"
            ),
            ErrorKind::ProjectCorrupted => (
                StatusCode::BAD_REQUEST,
                "Tried to get project into a ready state for deletion but failed. Please reach out to Klyra support for help."
            ),
            ErrorKind::ProjectHasResources(resources) => {
                let resources = resources.join(", ");
                return Self {
                    message: format!("Could not automatically delete the following resources: {}. Please reach out to Klyra support for help.", resources),
    ...
    fn from(code: StatusCode) -> Self {
        let message = match code {
            ...
            StatusCode::UNAUTHORIZED => {
                "we were unable to authorize your request. Check that your API key is set correctly. Use `cargo klyra login` to set it."
            },
...
// Note: The string "Invalid project name" is used by cargo-klyra to determine what type of error was returned.
...
pub mod axum {
    ...
    /// which is what the [`klyra_common::project::ProjectName`] parser uses.
