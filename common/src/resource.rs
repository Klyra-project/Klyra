...
/// Return this struct as a resource config to make Klyra provision it
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ProvisionResourceRequest {
    /// The version of the config+data schema for this Klyra resource
    pub version: u32,
    ...
    fn from(value: ProvisionResourceRequest) -> Self {
        Self {
            r#type: match value.r#type {
                ...
                r => panic!("Resource not supported on klyra.dev: {r}"),
            },
            config: value.config,
        }
    }
...
#[derive(Deserialize)]
#[serde(untagged)] // Try deserializing as a Klyra resource, fall back to a custom value
pub enum ResourceInput {
    Klyra(ProvisionResourceRequest),
    Custom(Value),
}
...
pub enum ResourceInputBeta {
    Klyra(ProvisionResourceRequestBeta),
    Custom(Value),
}
...
pub struct KlyraResourceOutput<T> {
    /// The output type for this Klyra resource,
