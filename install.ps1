function Install-Cargo-Klyra {
    ...
    if ($env:DO_NOT_TRACK -eq "1" -or $env:DO_NOT_TRACK -eq "true" -or
        ...
        $env:klyra_DISABLE_TELEMETRY -eq "1" -or $env:klyra_DISABLE_TELEMETRY -eq "true" -or
...
"@
    if ($TELEMETRY -eq "1") {
        ...
        Write-Host "https://docs.klyra.dev/install-script" -ForegroundColor Gray
    }
    Write-Host "==================================================="
    Write-Host
    ...
    function Send-Telemetry {
        if ($TELEMETRY -eq "1") {
            ...
            Invoke-RestMethod -Uri "https://console.klyra.dev/ingest/i/v0/e" -Method Post -ContentType "application/json" -Body $telemetry_data -ErrorAction SilentlyContinue | Out-Null
        }
    }
    ...
    function Exit-Success {
        ...
        Send-Telemetry
        Write-Host ""
        Write-Host "Thanks for installing Klyra CLI!" -ForegroundColor Green
    }
    ...
    function Exit-Failure {
        ...
        $OUTCOME = "failure"
        Write-Host ""
        Write-Host "Klyra installation script failed with reason: $STEP_FAILED" -ForegroundColor Red
        Write-Host "If you have any problems, please open an issue on GitHub or visit our Discord!"
        Send-Telemetry
    }
    ...
    if (Get-Command -CommandType Application -ErrorAction SilentlyContinue cargo-klyra.exe) {
        $NEW_INSTALL = "false"
        $LatestReleaseStripped = $LatestRelease -replace '^v', ''
        $CurrentVersion = & cargo-klyra.exe -V
        $CurrentVersionStripped = $CurrentVersion -replace '^cargo-klyra ', ''
        if ($LatestReleaseStripped -eq $CurrentVersionStripped) {
            Write-Host "Klyra CLI is already at the latest version!" -ForegroundColor Green
            return
        }
        else {
            Write-Host "Updating Klyra CLI to $LatestRelease"
        }
    }
    ...
    if (Get-Command -CommandType Application -ErrorAction SilentlyContinue cargo-binstall.exe) {
        Write-Host "Installing Klyra CLI using cargo-binstall"
        $INSTALL_METHOD = "cargo-binstall"
        cargo-binstall.exe -y --force --locked cargo-klyra
        if (!$?) {
    ...
    if (($Arch -eq "AMD64") -and (Get-Command -CommandType Application -ErrorAction SilentlyContinue tar.exe)) {
        ...
        New-Item -ItemType Directory -Force "$TempDir\cargo-klyra" | Out-Null
        ...
        Write-Host "Installing to $CargoHome\bin\cargo-klyra.exe"
        ...
        Write-Host "Installing to $CargoHome\bin\klyra.exe"
    ...
    cargo.exe install --locked cargo-klyra
