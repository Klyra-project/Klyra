...
_install_alpine_linux() {
  ...
  if command -v apk &>/dev/null; then
    if apk search -q cargo-klyra; then
      echo "cargo-klyra is not available in the testing repository. Do you want to enable the testing repository? (y/n)"
    ...
    if ! apk info cargo-klyra; then
      echo "Installing cargo-klyra"
      apk add cargo-klyra@testing
    else
...
_install_binary() {
  ...
  *)
  ...
  curl -LO "$REPO_URL/releases/download/$LATEST_VERSION/cargo-klyra-$LATEST_VERSION-$target.tar.gz"
...
_install_unsupported() {
  ...
  if command -v cargo-binstall &>/dev/null; then
    ...
    cargo binstall -y --locked cargo-klyra
    return 0
  fi
...
if command -v cargo-klyra &>/dev/null; then
  if [[ "$(cargo-klyra -V)" = *"${LATEST_VERSION#v}" ]]; then
    echo "cargo-klyra is already at the latest version!"
    exit
  else
    echo "Updating cargo-klyra to $LATEST_VERSION"
  fi
fi
...
cat <<EOF
Thanks for installing cargo-klyra! 🚀

If you have any issues, please open an issue on GitHub or visit our Discord (https://discord.gg/klyra)!
EOF
