...
if [[ \
    ...
    "${klyra_DISABLE_TELEMETRY:-}" == "1" || "${klyra_DISABLE_TELEMETRY:-}" == "true" || \
    "${CI:-}" == "1" || "${CI:-}" == "true"
  ]]; then
  TELEMETRY=0
fi
...
echo "\
https://docs.klyra.dev
https://discord.gg/klyra
...
if [[ "$TELEMETRY" = "1" ]]; then
  ...
  echo "https://docs.klyra.dev/install-script"
...
_send_telemetry() {
  ...
  \"properties\":{
    ...
    [ -n "${klyra_DEBUG:-}" ] && echo -e "DEBUG: Sending telemetry data:\n$telemetry_data"
    curl -sL -H 'Content-Type: application/json' -d "$telemetry_data" https://console.klyra.dev/ingest/i/v0/e > /dev/null
  fi
}
...
_exit_success() {
  ...
  echo "Thanks for installing Klyra CLI! 🚀"
  [[ "$SUPPORTS_COLOR" = "1" ]] && echo -en "\e[0m"
  if [[ "$INSTALLED_RUST" = "yes" ]]; then
    echo
    [[ "$SUPPORTS_COLOR" = "1" ]] && echo -en "\e[33m" # yellow
    echo "Remember to 'source \"$HOME/.cargo/env\"' or restart your shell to run cargo and klyra commands!"
    [[ "$SUPPORTS_COLOR" = "1" ]] && echo -en "\e[0m"
  fi
  exit 0
}
...
_exit_failure() {
  ...
  echo "Klyra installation script failed with reason: $STEP_FAILED"
  [[ "$SUPPORTS_COLOR" = "1" ]] && echo -en "\e[0m"
  echo "If you have any problems, please open an issue on GitHub or visit our Discord!"
  _send_telemetry
  exit 1
...
REPO_URL="https://github.com/klyra-hq/klyra"
...
if command -v cargo-klyra &>/dev/null; then
  NEW_INSTALL="false"
  if [[ "$(cargo-klyra -V)" = *"${LATEST_VERSION#v}" ]]; then
    [[ "$SUPPORTS_COLOR" = "1" ]] && echo -en "\e[32m" # green
    echo "Klyra CLI is already at the latest version!"
    [[ "$SUPPORTS_COLOR" = "1" ]] && echo -en "\e[0m"
    exit 0 # skip telemetry and instantly exit
  else
    echo "Updating Klyra CLI to $LATEST_VERSION"
  fi
fi
...
_install_linux() {
  ...
  if (uname -a | grep -qi "Microsoft"); then
    ...
    [ -n "${klyra_DEBUG:-}" ] && echo "DEBUG: lsb_release could not be found. Falling back to /etc/os-release"
...
_install_arch_linux() {
  ...
  if command -v pacman &>/dev/null; then
    ...
    if [[ "${pacman_version}" != "${LATEST_VERSION#v}"* ]]; then
      ...
      sudo pacman -S --noconfirm cargo-klyra || _exit_failure "pacman-install"
    fi
  else
...
_install_alpine_linux() {
  ...
  if command -v apk &>/dev/null; then
    if [ -z "$(apk search -q cargo-klyra)" ]; then
      while true; do
        read -r -p "cargo-klyra apk package not found. Do you want to enable the testing repository? [Y/n] " yn </dev/tty
        case $yn in
        [Yy]*|"")
    ...
    if ! apk info -q --installed cargo-klyra; then
      echo "Installing with apk"
      INSTALL_METHOD="apk"
      apk add cargo-klyra@testing || _exit_failure "apk-add"
    else
      ...
      if [[ "${apk_version}" != "${LATEST_VERSION#v}"* ]]; then
        ...
        echo "Installing with apk"
        INSTALL_METHOD="apk"
        apk upgrade cargo-klyra || _exit_failure "apk-upgrade"
      fi
    fi
  else
...
_install_binary() {
  ...
  *)
  ...
  curl -LO "$REPO_URL/releases/download/$LATEST_VERSION/cargo-klyra-$LATEST_VERSION-$target.tar.gz" || _exit_failure "download-binary"
  if ! command -v tar &>/dev/null; then
    _exit_failure "tar-not-found"
  fi
  tar -xzf "cargo-klyra-$LATEST_VERSION-$target.tar.gz" || _exit_failure "tar-extract-binary"
  echo "Installing to $HOME/.cargo/bin/cargo-klyra"
  mv "cargo-klyra-$target-$LATEST_VERSION/cargo-klyra" "$HOME/.cargo/bin/" || _exit_failure "move-binary"
  echo "Installing to $HOME/.cargo/bin/klyra"
  mv "cargo-klyra-$target-$LATEST_VERSION/klyra" "$HOME/.cargo/bin/" || _exit_failure "move-binary"
  popd >/dev/null || _exit_failure "popd"
  if [[ ":$PATH:" != *":$HOME/.cargo/bin:"* ]]; then
    echo "Add $HOME/.cargo/bin to PATH to access the 'klyra' command"
  fi
}
...
_install_with_cargo() {
  ...
  cargo install --locked cargo-klyra || _exit_failure "cargo-install"
}
...
_install_default() {
  ...
  if command -v cargo-binstall &>/dev/null; then
    ...
    cargo-binstall -y --force --locked cargo-klyra || _exit_failure "cargo-binstall"
    return 0
  fi
