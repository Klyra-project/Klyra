...
use klyra_service::{
    error::{CustomError, Error as KlyraServiceError},
...
impl<'a> ResourceBuilder<PathBuf> for StaticFolder<'a> {
    ...
    async fn build(build_data: &Self::Output) -> Result<PathBuf, klyra_service::Error> {
...
impl From<Error> for klyra_service::Error {
    fn from(error: Error) -> Self {
        ...
        KlyraServiceError::Custom(CustomError::msg(msg))
    }
}
...
mod tests {
    ...
    use async_trait::async_trait;
    use klyra_service::{DatabaseReadyInfo, Factory, ResourceBuilder};
