...
impl From<Error> for klyra_service::Error {
    fn from(error: Error) -> Self {
        let msg = match error {
            Error::UrlParseError(err) => format!("Cannot parse Turso Url: {}", err),
        };

        KlyraError::Custom(CustomError::msg(msg))
    }
}
...
impl Turso {
    async fn output_from_addr(
        &self,
        addr: &str,
    ) -> Result<<Turso as ResourceBuilder<Client>>::Output, klyra_service::Error> {
        Ok(TursoOutput {
...
impl ResourceBuilder<Client> for Turso {
    ...
    ) -> Result<Self::Output, klyra_service::Error> {
        match factory.get_environment() {
            klyra_service::Environment::Production => {
                if self.addr.is_empty() {
                    Err(KlyraError::Custom(CustomError::msg("missing addr")))
                } else {
    ...
    async fn build(config: &Self::Output) -> Result<Client, klyra_service::Error> {
...
mod test {
    ...
    use async_trait::async_trait;
    use klyra_service::{DatabaseReadyInfo, Environment, Factory, ResourceBuilder, ServiceName};
    ...
    impl MockFactory {
        fn new() -> Self {
            Self {
                temp_dir: Builder::new().prefix("klyra-turso").tempdir().unwrap(),
                service_name: "klyra-turso".to_string(),
    ...
    impl Factory for MockFactory {
        async fn get_db_connection(
            &mut self,
            _db_type: klyra_service::database::Type,
        ...
        async fn get_secrets(
            &mut self,
        ) -> Result<std::collections::BTreeMap<String, String>, klyra_service::Error> {
        ...
        fn get_environment(&self) -> klyra_service::Environment {
            self.environment
        }

        fn get_build_path(&self) -> Result<std::path::PathBuf, klyra_service::Error> {
            Ok(self.build_path())
        }

        fn get_storage_path(&self) -> Result<std::path::PathBuf, klyra_service::Error> {
