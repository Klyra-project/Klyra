...
impl Turso {
    async fn output_from_addr(&self, addr: &str) -> Result<TursoOutput, klyra_service::Error> {
        Ok(TursoOutput {
...
impl IntoResource<Client> for TursoOutput {
    async fn into_resource(self) -> Result<Client, klyra_service::Error> {
...
mod test {
    ...
    impl Factory for MockFactory {
        ...
        ) -> Result<klyra_service::DatabaseInfo, klyra_service::Error> {
            panic!("no turso test should try to get a db connection string")
        }

        async fn get_container(
            &mut self,
            _req: klyra_service::ContainerRequest,
        ) -> Result<klyra_service::ContainerResponse, klyra_service::Error> {
            panic!("no turso test should try to get a container")
        }

        async fn get_secrets(
            &mut self,
        ) -> Result<std::collections::BTreeMap<String, Secret<String>>, klyra_service::Error>
        {
            panic!("no turso test should try to get secrets")
        }

        fn get_metadata(&self) -> klyra_service::DeploymentMetadata {
            klyra_service::DeploymentMetadata {
