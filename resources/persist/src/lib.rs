...
use klyra_service::{Factory, ResourceBuilder, ServiceName, Type};
use std::fs;
use std::fs::File;
...
impl PersistInstance {
    ...
    fn get_storage_folder(&self) -> PathBuf {
        ["klyra_persist", &self.service_name.to_string()]
            .iter()
            .collect()
    }
...
impl ResourceBuilder<PersistInstance> for Persist {
    ...
    async fn output(
        ...
        let persist_instance = match PersistInstance::new(factory.get_service_name()) {
            ...
            Err(e) => return Err(klyra_service::Error::Custom(e.into())),
        };
        Ok(persist_instance)
    }

    async fn build(build_data: &Self::Output) -> Result<PersistInstance, klyra_service::Error> {
        Ok(build_data.clone())
    }
}
