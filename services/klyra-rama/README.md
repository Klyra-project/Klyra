## Klyra service integration for the Rama framework
...
#[klyra_runtime::main]
async fn main() -> Result<impl klyra_rama::KlyraService, klyra_rama::KlyraError> {
    let router = Router::new().get("/", hello_world);

    let app =
        // Klyra sits behind a load-balancer,
        // so in case you want the real IP of the user,
        // you need to ensure this headers is handled.
        //
        // Learn more at <https://docs.klyra.dev/docs/deployment-environment#https-traffic>
        GetForwardedHeaderLayer::x_forwarded_for().into_layer(router);

    Ok(klyra_rama::RamaService::application(app))
}
```
...
#[klyra_runtime::main]
async fn main() -> Result<impl klyra_rama::KlyraService, klyra_rama::KlyraError> {
    Ok(klyra_rama::RamaService::transport(service_fn(
        hello_world,
    )))
}
```
