...
async fn shorten(url: String, state: &State<AppState>) -> Result<String, status::Custom<String>> {
    ...
    Ok(format!("https://s.klyraapp.rs/{id}"))
}
...
async fn build_state(factory: &mut dyn Factory) -> Result<AppState, klyra_service::Error> {
    ...
    MIGRATOR
        .run(&pool)
        .await
        .map_err(|err| klyra_service::Error::Database(sqlx::Error::Migrate(Box::new(err))))?;
