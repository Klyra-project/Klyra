...
use klyra_proto::resource_recorder::Client;
use klyra_proto::resource_recorder::ProjectResourcesRequest;
...
where
    T: ResourceDal,
{
    #[instrument(skip_all, fields(klyra.project.id = project_id))]
    async fn get_project_resources(
        &mut self,
        project_id: &str,
        token: &str,
...
impl ResourceDal for Client {
    async fn get_project_resources(
        ...
        project_id: &str,
        token: &str,
    ) -> Result<Vec<klyra_common::resource::Response>, Error> {
...
mod tests {
    ...
    use serde_json::json;
    use klyra_common::{database, resource};
    use klyra_proto::resource_recorder::{get_client, record_request, Client, RecordRequest};
