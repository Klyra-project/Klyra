...
impl PermissionsDal for Client {
    #[instrument(skip_all, fields(klyra.user.id = user_id, klyra.permit.client = true))]
    async fn get_user(&self, user_id: &str) -> Result<UserRead> {
    ...
    #[instrument(skip_all, fields(
        klyra.user.id = user_id,
        klyra.project.id = project_id,
        klyra.permit.client = true,
    ))]
    async fn create_project(&self, user_id: &str, project_id: &str) -> Result<()> {
    ...
    #[instrument(skip_all, fields(klyra.project.id = project_id, klyra.permit.client = true))]
    async fn delete_project(&self, project_id: &str) -> Result<()> {
