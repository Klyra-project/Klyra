    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
                ...
                .map(|o| {
                    let value = match &o.value {
                        Expr::Lit(ExprLit {
                            ...
                            needs_vars = true;
                            quote!(&::klyra_runtime::__internals::strfmt(#str, &vars)?)
                        }
                        other => quote!(#other),
                    };
        ...
        let loader = quote! {
            async fn loader(
                ...
                #extra_imports
                #vars
                #(let #fn_inputs = ::klyra_runtime::__internals::get_resource(
    ...
    fn output_with_inputs() {
        ...
        let expected = quote! {
            async fn loader(
                ...
                use ::klyra_runtime::__internals::Context;
                use ::klyra_runtime::{Factory, ResourceBuilder};
                let pool = ::klyra_runtime::__internals::get_resource(
                    klyra_shared_db::Postgres::new(),
                    &mut factory,
                    &mut resource_tracker,
                ).await.context(format!("failed to provision {}", stringify!(klyra_shared_db::Postgres)))?;
                let redis = ::klyra_runtime::__internals::get_resource(
                    klyra_shared_db::Redis::new(),
                    &mut factory,
                    &mut resource_tracker,
                ).await.context(format!("failed to provision {}", stringify!(klyra_shared_db::Redis)))?;
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn loader(
                ...
                let pool = ::klyra_runtime::__internals::get_resource (
                    klyra_shared_db::Postgres::new().size(&::klyra_runtime::__internals::strfmt("10Gb", &vars)?).public(false),
                    &mut factory,
                    &mut resource_tracker,
                ).await.context(format!("failed to provision {}", stringify!(klyra_shared_db::Postgres)))?;
                std::mem::drop(vars);
