        ...
        let inputs: Vec<_> = item_fn
            ...
            .filter_map(|(pat_ident, attrs, ty)| {
                match attribute_to_builder(pat_ident, attrs) {
                    ...
                    Err(err) => {
                        emit_error!(pat_ident, err; hint = pat_ident.span() => "Try adding a config like `#[klyra_shared_db::Postgres]`");
                        None
                    }
                }
            })
...
impl ToTokens for Loader {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
                ...
                .map(|o| {
                    let value = match &o.value {
                        Expr::Lit(ExprLit {
                            ...
                            needs_vars = true;
                            quote!(&::klyra_runtime::__internals::strfmt(#str, &__vars)?)
                        }
                        other => quote!(#other),
                    };
        ...
        let loader_runner = quote! {
            ...
            ) -> Result<Vec<Vec<u8>>, ::klyra_runtime::Error> {
