...
pub(crate) fn tokens(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        fn main() {
            ...
            ::klyra_runtime::tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .build()
                .unwrap()
                .block_on(async {
                    ::klyra_runtime::__internals::start(__loader, __runner).await;
                })
        }
...
impl LoaderAndRunner {
    /// Modifies function identifier and inputs while extracting and constructing the Klyra inputs
    pub(crate) fn from_item_fn(item_fn: &mut ItemFn) -> Option<Self> {
        ...
        item_fn.sig.ident = Ident::new(
            &format!("__klyra_{}", item_fn.sig.ident),
            Span::call_site(),
        );
        ...
        let inputs: Vec<_> = item_fn
            ...
            .filter_map(|(pat_ident, attrs, ty)| {
                match Self::attribute_to_builder(pat_ident, attrs) {
                    ...
                    Err(err) => {
                        emit_error!(pat_ident, err; hint = pat_ident.span() => "Try adding an attribute like `#[klyra_shared_db::Postgres]`");
                        None
                    }
                }
            })
        ...
        match signature.output {
            ReturnType::Default => {
                emit_error!(
                    ...
                    doc = "https://docs.rs/klyra-runtime/latest/klyra_runtime/attr.main.html#klyra-supported-services"
                );
                None
            }
            ReturnType::Type(_, ty) => match *ty {
                ...
                _ => {
                    emit_error!(
                        ty,
                        "klyra_runtime::main functions need to return a first class service or 'Result<impl klyra_service::Service, klyra_runtime::Error>";
                        hint = "See the docs for services with first class support";
                        doc = "https://docs.rs/klyra-runtime/latest/klyra_runtime/attr.main.html#klyra-supported-services"
                    );
                    None
    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
                ...
                .map(|o| {
                    let value = match &o.value {
                        Expr::Lit(ExprLit {
                            ...
                            needs_vars = true;
                            quote!(&::klyra_runtime::__internals::strfmt(#string, &__vars)?)
                        }
                        other => quote!(#other),
                    };
        ...
        let (factory_ident, extra_imports): (Ident, Option<Stmt>) = if self.fn_inputs.is_empty() {
            ...
            (
                parse_quote!(factory),
                Some(parse_quote!(
                    use ::klyra_runtime::{ResourceFactory, IntoResource, ResourceInputBuilder};
                )),
            )
        };
        ...
        let loader_runner = quote! {
            async fn __loader(
                #factory_ident: ::klyra_runtime::ResourceFactory,
            ) -> ::std::result::Result<::std::vec::Vec<::std::vec::Vec<::core::primitive::u8>>, ::klyra_runtime::Error> {
                use ::klyra_runtime::__internals::Context;
                #extra_imports
                ...
                #(
                    ...
                    let json = ::klyra_runtime::__internals::serde_json::to_vec(&input)
            ...
            async fn __runner(
                ...
                #(
                    let x: <#fn_input_builders as ResourceInputBuilder>::Output =
                        ::klyra_runtime::__internals::serde_json::from_slice(
...
mod tests {
    ...
    fn from_with_return() {
        ...
        let expected_ident: Ident = parse_quote!(__klyra_simple);
        let expected_return: TypePath = parse_quote!(KlyraAxum);
    ...
    fn from_with_main() {
        ...
        let actual = LoaderAndRunner::from_item_fn(&mut input).unwrap();
        let expected_ident: Ident = parse_quote!(__klyra_main);
    ...
    fn parse_fn_inputs() {
        ...
        let actual = LoaderAndRunner::from_item_fn(&mut input).unwrap();
        let expected_ident: Ident = parse_quote!(__klyra_complex);
        let expected_inputs: Vec<Input> = vec![Input {
            ident: parse_quote!(pool),
            builder: BuilderPattern {
                path: parse_quote!(klyra_shared_db::Postgres),
    ...
    fn loader_runner_simple_inputs() {
        ...
        let expected = quote! {
            async fn __loader(
                _factory: ::klyra_runtime::ResourceFactory,
            ) -> ::std::result::Result<::std::vec::Vec<::std::vec::Vec<::core::primitive::u8>>, ::klyra_runtime::Error> {
                use ::klyra_runtime::__internals::Context;
                let mut inputs = Vec::new();
                Ok(inputs)
    ...
    fn loader_runner_complex_inputs() {
        let input = LoaderAndRunner {
            fn_ident: parse_quote!(__klyra_complex),
            fn_inputs: vec![
                Input {
                    ident: parse_quote!(pool),
                    builder: BuilderPattern {
                ...
                Input {
                    ...
                    builder: BuilderPattern {
                        path: parse_quote!(klyra_shared_db::Redis),
                        options: Default::default(),
                    },
        ...
        let expected = quote! {
            async fn __loader(
                factory: ::klyra_runtime::ResourceFactory,
            ) -> ::std::result::Result<::std::vec::Vec<::std::vec::Vec<::core::primitive::u8>>, ::klyra_runtime::Error> {
                use ::klyra_runtime::__internals::Context;
                use ::klyra_runtime::{ResourceFactory, IntoResource, ResourceInputBuilder};
                let mut inputs = Vec::new();
                let input: <klyra_shared_db::Postgres as ResourceInputBuilder>::Input =
                    klyra_shared_db::Postgres::default()
                    .build(&factory)
                    .await
                    .context(format!("failed to construct config for {}", stringify!(klyra_shared_db::Postgres)))?;
                let json = ::klyra_runtime::__internals::serde_json::to_vec(&input)
                    .context(format!("failed to serialize config for {}", stringify!(klyra_shared_db::Postgres)))?;
                inputs.push(json);
                let input: <klyra_shared_db::Redis as ResourceInputBuilder>::Input =
                    klyra_shared_db::Redis::default()
                    .build(&factory)
                    .await
                    .context(format!("failed to construct config for {}", stringify!(klyra_shared_db::Redis)))?;
                let json = ::klyra_runtime::__internals::serde_json::to_vec(&input)
                    .context(format!("failed to serialize config for {}", stringify!(klyra_shared_db::Redis)))?;
                inputs.push(json);
                Ok(inputs)
            }
            ...
            async fn __runner(
                ...
                use ::klyra_runtime::{ResourceFactory, IntoResource, ResourceInputBuilder};
                let mut iter = resources.into_iter();
                let x: <klyra_shared_db::Postgres as ResourceInputBuilder>::Output =
                    ::klyra_runtime::__internals::serde_json::from_slice(
                        &iter.next().expect("resource list to have correct length")
                    )
                    .context(format!("failed to deserialize output for {}", stringify!(klyra_shared_db::Postgres)))?;
                let pool: sqlx::PgPool = x.into_resource()
                    .await
                    .context(format!("failed to initialize {}", stringify!(klyra_shared_db::Postgres)))?;
                let x: <klyra_shared_db::Redis as ResourceInputBuilder>::Output =
                    ::klyra_runtime::__internals::serde_json::from_slice(
                        &iter.next().expect("resource list to have correct length")
                    )
                    .context(format!("failed to deserialize output for {}", stringify!(klyra_shared_db::Redis)))?;
                let redis: something::Redis = x.into_resource()
                    .await
                    .context(format!("failed to initialize {}", stringify!(klyra_shared_db::Redis)))?;

                __klyra_complex(pool, redis).await
            }
        };
    ...
    fn output_with_input_options() {
        ...
        let expected = quote! {
            async fn __loader(
                ...
                let input: <klyra_shared_db::Postgres as ResourceInputBuilder>::Input =
                    klyra_shared_db::Postgres::default()
                    .size(&::klyra_runtime::__internals::strfmt("10Gb", &__vars)?).public(false)
                    .build(&factory)
                    .await
                    .context(format!("failed to construct config for {}", stringify!(klyra_shared_db::Postgres)))?;
                let json = ::klyra_runtime::__internals::serde_json::to_vec(&input)
