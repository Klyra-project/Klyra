...
impl ToTokens for Endpoint {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let route = quote!(.route(#route, klyra_next::routing::#method(#function)));
...
pub(crate) fn wasi_bindings(app: App) -> proc_macro2::TokenStream {
    quote!(
        ...
        pub extern "C" fn __klyra_Axum_call(
            ...
            let res = klyra_next::block_on(__app(request));

            let (parts, mut body) = res.into_parts();

            // wrap and serialize response parts as rmp
            let response_parts = klyra_next::ResponseWrapper::from(parts)
                .into_rmp()
...
mod tests {
    ...
    fn endpoint_to_token() {
        ...
        let expected = quote!(.route("/hello", klyra_next::routing::get(hello)));
    ...
    fn app_to_token() {
        ...
        let expected = quote!(
            async fn __app(
                request: klyra_next::Request<klyra_next::body::BoxBody>,
            ) -> klyra_next::response::Response {
                use klyra_next::Service;

                let mut router = klyra_next::Router::new()
                    .route("/hello", klyra_next::routing::get(hello))
                    .route("/goodbye", klyra_next::routing::post(goodbye));
    ...
    fn parse_app() {
        let mut input = parse_quote! {
            ...
            #[klyra_codegen::endpoint(method = post, route = "/goodbye")]
