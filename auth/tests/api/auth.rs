mod needs_docker {
    ...
    async fn convert_api_key_to_jwt() {
        ...
        // GET /auth/key with a basic user key that has an XKlyraAdminSecret header with a basic user key.
        let response = app
            .get_jwt_from_api_key(basic_user_key, Some(basic_user_key))
            .await;
        assert_eq!(response.status(), StatusCode::UNAUTHORIZED);

        // GET /auth/key with an admin user key that has an XKlyraAdminSecret header with a basic user key.
        let response = app
        ...
        // GET /auth/key with a basic user key that has an XKlyraAdminSecret header with an admin user key.
        let response = app
