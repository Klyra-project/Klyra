...
use klyra_common::{backends::auth::Claim, models::user};
...
pub(crate) async fn convert_cookie(
...
) -> Result<Json<klyra_common::backends::auth::ConvertResponse>, StatusCode> {
    ...
    let response = klyra_common::backends::auth::ConvertResponse { token };
