...
use klyra_common::{backends::headers::XKlyraAdminSecret, claims::AccountTier, ApiKey, Secret};
...
impl UserManagement for UserManager {
    ...
    async fn upgrade_to_pro(
            ...
            .is_some()
        {
            ...
            .bind(name)
            .bind(KlyraSubscriptionType::Pro.to_string())
            .execute(&mut *transaction)
            .await?;
...
pub struct Subscription {
    ...
    pub r#type: KlyraSubscriptionType,
    pub quantity: i32,
...
pub enum KlyraSubscriptionType {
    Pro,
    Rds,
}
...
impl User {
    ...
    pub fn pro_subscription_id(&self) -> Option<&stripe::SubscriptionId> {
        self.subscriptions
            .iter()
            .find(|sub| matches!(sub.r#type, KlyraSubscriptionType::Pro))
            .map(|sub| &sub.id)
    }
    ...
    fn from_row(row: &PgRow) -> Result<Self, sqlx::Error> {
        Ok(Subscription {
            id: row
                ...
                .unwrap(),
            r#type: KlyraSubscriptionType::from_str(row.try_get("type").unwrap()).map_err(
                |err| sqlx::Error::ColumnDecode {
    ...
    UserManagerState: FromRef<S>,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        match parts.headers.typed_try_get::<XKlyraAdminSecret>() {
            Ok(Some(secret)) => {
