...
use klyra_proto::runtime::{self, runtime_client::RuntimeClient, StopRequest};
        ...
        let get_runtime_executable = || {
            if let Some(alpha_runtime) = alpha_runtime_path {
                ...
                if cfg!(debug_assertions) {
                    debug!("Installing klyra-next runtime in debug mode from local source");
                    // If we're running deployer natively, install klyra-runtime using the
                    ...
                    if let Ok(path) = path {
                        std::process::Command::new("cargo")
                            ...
                            .arg("next")
                            .output()
                            .expect("failed to install the local version of klyra-runtime");
                    }
                }

                debug!("Returning path to klyra-next runtime",);
