...
const BUILDS_PATH: &str = "/tmp/klyra-builds";
...
/// when built.
const MARKER_FILE_NAME: &str = ".klyra-marker";
...
mod tests {
    ...
    async fn extract_tar_gz_data() {
        let dir = TempDir::new("/tmp/klyra-extraction-test").unwrap();
        let p = dir.path();
...
ff0e55bda1ff01000000000000000000e0079c01ff12a55500280000",
    ...
    async fn remove_old_build() {
        let dir = TempDir::new("/tmp/klyra-remove-old-test").unwrap();
        let p = dir.path();
    ...
    async fn rename_build() {
        let dir = TempDir::new("/tmp/klyra-rename-build-test").unwrap();
        let p = dir.path();
