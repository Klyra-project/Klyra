                            ...
                            tokio::spawn(async move {
                                ...
                                match inner.build(req).await {
                                    Ok(inner) =>  {
                                        ...
                                        info!(id = %queued.id, "klyra-builder finished building the deployment: image length is {} bytes, is_wasm flag is {} and there are {} secrets", response.image.len(), response.is_wasm, response.secrets.len());
                                    },
                                    Err(err) => error!(id = %queued.id, "klyra-builder errored while building: {}", err)
                                };
                            });
                        }
    ...
    async fn handle(
        ...
        tokio::task::spawn_blocking(move || {
            while let Ok(message) = rx.recv() {
                ...
                let log = LogItem::new(
                    self.id,
                    klyra_common::log::Backend::Deployer, // will change to Builder
                    match message {
        ...
        // Currently returns the first found klyra service in a given workspace.
        let built_service = build_deployment(&project_path, tx.clone()).await?;

        // Get the Secrets.toml from the klyra service in the workspace.
...
async fn extract_tar_gz_data(data: impl Read, dest: impl AsRef<Path>) -> Result<()> {
    ...
    for entry in archive.entries()? {
        ...
        // don't allow archive to overwrite klyra internals
...
/// an embedded runtime for alpha, and a .wasm file for klyra-next.
