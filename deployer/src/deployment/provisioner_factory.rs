...
impl<R: ResourceManager, S: SecretGetter> Factory for ProvisionerFactory<R, S> {
    async fn get_db_connection_string(
        ...
        let info = if let Some(claim) = self.claim.clone() {
            info!("Provisioning a {db_type} on the klyra servers. This can take a while...");
            ...
            self.resource_manager
                .insert_resource(&Resource {
                    ...
                    data: serde_json::to_value(&info).map_err(|err| {
                        klyra_service::Error::Database(format!(
                ...
                .await
                .map_err(|err| {
                    klyra_service::Error::Database(format!("failed to store resource: {err}"))
                })?;
            ...
            let resources = self
                ...
                .await
                .map_err(|err| {
                    klyra_service::Error::Database(format!("failed to get resources: {err}"))
                })?;
            ...
            if let Some(info) = info {
                ...
                return Err(klyra_service::Error::Database(
    ...
    async fn get_secrets(&mut self) -> Result<BTreeMap<String, String>, klyra_service::Error> {
