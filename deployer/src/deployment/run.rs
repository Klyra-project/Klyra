...
use klyra_service::{Environment, KlyraResourceOutput};
use tokio::{
    sync::Mutex,
...
impl Built {
    ...
    pub async fn handle(
        ...
        // For klyra-next this is the path to the compiled .wasm file, which will be
        ...
        let alpha_runtime_path = if self.is_next {
            // The runtime client for next is the installed klyra-next bin
            None
        } else {
        ...
        let prev_resources = resource_manager
            ...
            .inspect(|r| {
                if r.r#type == klyra_common::resource::Type::Secrets {
...
async fn load(
    ...
    debug!(klyra.project.name = %service_name, "loading service");
    ...
    debug!(klyra.project.name = %service_name, "service loaded");
    match response {
        Ok(response) => {
...
fn get_cached_output<T: DeserializeOwned>(
    klyra_resource: &ProvisionResourceRequest,
    prev_resources: &[resource::Response],
) -> Option<T> {
    prev_resources
        .iter()
        .find(|resource| {
            resource.r#type == klyra_resource.r#type && resource.config == klyra_resource.config
        })
        .and_then(|resource| {
            let cached_output = resource.data.clone();
            log(klyra_resource.r#type, "Found cached output");
            match serde_json::from_value::<T>(cached_output) {
                Ok(output) => Some(output),
                Err(_) => {
                    log(klyra_resource.r#type, "Failed to validate cached output");
                    None
                }
            }
        })
            ...
            }).collect::<anyhow::Result<Vec<_>>>()?.into_iter()
    {
        ...
        match klyra_resource.r#type {
            resource::Type::Database(db_type) => {
                ...
                let _config: DbInput = serde_json::from_value(klyra_resource.config.clone())
                    .context("deserializing resource config")?;

                let output = get_cached_output(&klyra_resource, prev_resources.as_slice());
                let output = match output {
                    Some(o) => o,
                    None => {
                        log(klyra_resource.r#type, "Provisioning...");
                        // ###
                ...
                resources_to_save.push(record_request::Resource {
                    r#type: klyra_resource.r#type.to_string(),
