...
use klyra_common::{backends::auth::Claim, project::ProjectName as ServiceName};
use klyra_service::{
...
mod tests {
    ...
    impl Factory for StubFactory {
        ...
        fn get_storage_path(&self) -> Result<PathBuf, klyra_service::Error> {
            panic!("no test should get the storage path");
        }
    }

    fn get_storage_manager() -> StorageManager {
        let tmp_dir = Builder::new().prefix("klyra_run_test").tempdir().unwrap();
