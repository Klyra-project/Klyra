...
async fn load(
    ...
    let cached_resources_contains_rds = resources.iter().any(|resource| {
        matches!(
            resource.r#type,
            klyra_common::resource::Type::Database(klyra_common::database::Type::AwsRds(_))
        )
    });
    ...
    match response {
        Ok(response) => {
            ...
            let loaded_resources_contains_rds = resources.iter().any(|resource| {
                matches!(
                    resource.r#type,
                    klyra_common::resource::Type::Database(
                        klyra_common::database::Type::AwsRds(_)
                    )
                )
            });
            ...
            if !cached_resources_contains_rds && loaded_resources_contains_rds {
                deployment_updater
                    .set_message(
                        &deployment_id,
                        "This deployment increased the cost of your subscription. To check your total, log in to the Klyra web console.",
                    )
                    .await
                    .map_err(|err| {
