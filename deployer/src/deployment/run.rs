...
use klyra_common::project::ProjectName as ServiceName;
use klyra_common::storage_manager::ArtifactsStorageManager;
use klyra_proto::runtime::{runtime_client::RuntimeClient, LoadRequest, StartRequest};
...
pub async fn task(
    ...
    while let Some(built) = recv.recv().await {
        ...
        let cleanup = move |result: std::result::Result<
            std::result::Result<(), klyra_service::Error>,
            JoinError,
        >| match result {
...
impl Built {
    ...
    async fn handle(
        ...
        cleanup: impl FnOnce(std::result::Result<std::result::Result<(), klyra_service::Error>, JoinError>)
            + Send
            + 'static,
    ) -> Result<()> {
...
async fn run(
    ...
    _cleanup: impl FnOnce(std::result::Result<std::result::Result<(), klyra_service::Error>, JoinError>)
        + Send
        + 'static,
) {
...
mod tests {
    ...
    fn get_storage_manager() -> ArtifactsStorageManager {
        let tmp_dir = TempDir::new("klyra_run_test").unwrap();
    ...
    async fn panic_in_bind() {
        ...
        >| {
            let result = result.unwrap();
            assert!(
                matches!(result, Err(klyra_service::Error::BindPanic(ref msg)) if msg == "panic in bind"),
    ...
    async fn panic_in_main() {
        ...
        assert!(
            matches!(result, Err(Error::Run(klyra_service::Error::BuildPanic(ref msg))) if msg == "main panic"),
    ...
    async fn missing_so() {
        ...
        assert!(
            matches!(
                result,
                Err(Error::Load(klyra_service::loader::LoaderError::Load(_)))
            ),
