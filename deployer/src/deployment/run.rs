...
fn get_cached_output<T: DeserializeOwned>(
    klyra_resource_type: &Type,
    config: &serde_json::Value,
    prev_resources: &[resource::Response],
) -> Option<T> {
    prev_resources
        .iter()
        .find(|resource| resource.r#type == *klyra_resource_type && resource.config == *config)
        .and_then(|resource| {
            let cached_output = resource.data.clone();
            log(klyra_resource_type, "Found cached output");
            match serde_json::from_value::<T>(cached_output) {
                Ok(output) => Some(output),
                Err(_) => {
                    log(klyra_resource_type, "Failed to validate cached output");
                    None
                }
            }
        })
            ...
            }).collect::<anyhow::Result<Vec<_>>>()?.into_iter()
    {
        ...
        match klyra_resource.r#type {
            resource::Type::Database(db_type) => {
                ...
                // be cached.
                let output = get_cached_output(&klyra_resource.r#type, &serde_json::Value::Null, prev_resources.as_slice());
