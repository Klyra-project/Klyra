...
use clap::Parser;
use klyra_deployer::{start, start_proxy, Args, DeployLayer, Persistence};
use klyra_proto::runtime::runtime_client::RuntimeClient;
use klyra_proto::runtime::SubscribeLogsRequest;
use tokio::select;
...
// The `multi_thread` is needed to prevent a deadlock in klyra_service::loader::build_crate() which spawns two threads
...
async fn main() {
    ...
    let mut runtime = tokio::process::Command::new(runtime_dir.join("klyra-runtime"))
        .args([
            "--legacy",
            "--provisioner-address",
