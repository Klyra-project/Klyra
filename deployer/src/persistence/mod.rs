...
use hyper::Uri;
use klyra_common::{claims::Claim, resource::Type};
use klyra_proto::{
...
impl ResourceManager for Persistence {
    ...
    async fn get_resource(
        &mut self,
        service_id: &Ulid,
        r#type: klyra_common::resource::Type,
        claim: Claim,
    ...
    async fn delete_resource(
        ...
        project_name: String,
        service_id: &Ulid,
        resource_type: klyra_common::resource::Type,
        claim: Claim,
    ) -> Result<ResultResponse> {
        if let Type::Database(db_type) = resource_type {
            let proto_db_type: klyra_proto::provisioner::database_request::DbType =
                db_type.into();
