...
#[openapi(
    ...
    components(schemas(
        ...
        klyra_common::log::Item,
        klyra_common::models::secret::Response,
        klyra_common::log::Level,
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Gets the logs a specific deployment.", body = [klyra_common::log::Item]),
        (status = 500, description = "Database or streaming error.", body = String),
        (status = 404, description = "Record could not be found.", body = String),
    ),
    params(
...
pub async fn get_logs(
    ...
    if let Ok(logs) = client.get_logs(logs_request).await {
        // TODO: awaits on the From impl for `klyra_proto::logger::LogItem` -> `klyra_common::LogItem`.
        Ok(Json(
            logs.into_inner()
                .log_items
                .into_iter()
                .map(klyra_common::LogItem::from)
                .collect(),
        ))
    } else {
        Err(Error::NotFound("deployment not found".to_string()))
