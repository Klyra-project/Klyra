...
use klyra_service::loader::{Loader, LoaderError};
...
fn not_klyra() {
    Command::new("cargo")
        .args(["build", "--release"])
        .current_dir("tests/resources/not-klyra")
        .spawn()
        .unwrap()
        .wait()
        .unwrap();
    ...
    let result =
        Loader::from_so_file("tests/resources/not-klyra/target/release/libnot_klyra.so");
