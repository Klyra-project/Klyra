...
fn not_klyra() {
    ...
    let project_path = format!("{}/tests/resources/not-klyra", env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(
        so_path
            .display()
            .to_string()
            .ends_with("tests/resources/not-klyra/target/debug/libnot_klyra.so"),
...
#[test]
#[should_panic(
    expected = "Your Klyra project must be a library. Please add `[lib]` to your Cargo.toml file."
)]
fn not_lib() {
    let buf = Box::new(DummyWriter {});
