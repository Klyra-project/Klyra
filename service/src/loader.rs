...
fn check_version(summary: &Summary) -> anyhow::Result<()> {
    ...
    if version_req.matches(&valid_version) {
        ...
        Err(anyhow!(
            "the version of `klyra-service` specified as a dependency to this service ({version_req}) is not supported by this instance ({valid_version}); try updating `klyra-service` to the latest version available and deploy"
        ))
    }
}
