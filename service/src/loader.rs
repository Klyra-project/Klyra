...
pub enum LoaderError {
    ...
    #[error("failed to find the klyra entrypoint. Did you use the provided klyra macros?")]
...
impl Loader {
    ...
    /// using the [`klyra_service::main`][crate::main] macro.
...
pub async fn build_crate(
    ...
    let current = ws.current_mut().map_err(|_| anyhow!("A Klyra project cannot have a virtual manifest file - please ensure your Cargo.toml file specifies it as a library."))?;
...
fn ensure_cdylib(manifest: &mut Manifest) -> anyhow::Result<()> {
        ...
        .find(|target| target.is_lib())
    {
        ...
        Err(anyhow!(
            "Your Klyra project must be a library. Please add `[lib]` to your Cargo.toml file."
        ))
    }
}
...
fn is_next(summary: &Summary) -> bool {
    let features = if let Some(klyra) = summary
        .dependencies()
        .iter()
        .find(|dependency| dependency.package_name() == "klyra-codegen")
    {
        klyra.features()
    } else {
        &[]
...
fn check_version(summary: &Summary) -> anyhow::Result<()> {
    ...
    let version_req = if let Some(klyra) = summary
        .dependencies()
        .iter()
    ...
    if version_req.matches(&valid_version) {
        ...
        Err(anyhow!(
            "the version of `klyra-service` specified as a dependency to this service ({version_req}) is not supported by this project instance ({valid_version}); try updating `klyra-service` to '{valid_version}' or update the project instance using `cargo klyra project rm` and `cargo klyra project new`"
        ))
    }
}
    ...
    if let Some(profiles) = ws.profiles() {
        for profile in profiles.get_all().values() {
            if profile.panic.as_deref() == Some("abort") {
                return Err(anyhow!("Your Klyra project cannot have panics that abort. Please ensure your Cargo.toml does not contain `panic = \"abort\"` for any profiles."));
            }
        }
    }
