...
/// This represents a compiled alpha or klyra-next service.
...
impl BuiltService {
    ...
    pub fn service_name(&self) -> anyhow::Result<ProjectName> {
        let klyra_toml_path = self.working_directory.join("Klyra.toml");
...
pub async fn build_workspace(
    ...
    if !manifest_path.exists() {
        return Err(anyhow!("failed to read the Klyra project manifest"));
    }
...
pub async fn clean_crate(project_path: &Path, release_mode: bool) -> anyhow::Result<Vec<String>> {
    ...
    let manifest_path = project_path.join("Cargo.toml");
    if !manifest_path.exists() {
        bail!("failed to read the Klyra project manifest");
    }
...
fn ensure_cdylib(package: &Package) -> anyhow::Result<()> {
    if package.targets.iter().any(is_cdylib) {
        ...
        bail!("Your Klyra next project must be a library. Please add `[lib]` to your Cargo.toml file.")
    }
}
