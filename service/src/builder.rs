...
pub async fn build_crate(
    ...
    let current = ws.current_mut().map_err(|_| anyhow!("A Klyra project cannot have a virtual manifest file - please ensure the `package` table is present in your Cargo.toml file."))?;
...
fn ensure_binary(manifest: &Manifest) -> anyhow::Result<()> {
    if manifest.targets().iter().any(|target| target.is_bin()) {
        ...
        bail!("Your Klyra project must be a binary.")
    }
}

/// Make sure "cdylib" is set for klyra-next projects, else set it if possible.
fn ensure_cdylib(manifest: &mut Manifest) -> anyhow::Result<()> {
        ...
        .find(|target| target.is_lib())
    {
        ...
        bail!("Your Klyra project must be a library. Please add `[lib]` to your Cargo.toml file.")
    }
}
