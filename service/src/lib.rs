...
///
/// Also see the [klyra_runtime::main] macro.
#[async_trait]
...
///
/// Use the [klyra_runtime::main] macro to expose your implementation to the deployment backend.
#[async_trait]
pub trait Service: Send + Clone {
    ...
    ///
    /// Default implementation assumes the service is bound to `addr` and responds with 200 OK on '/_klyra/healthz'.
    /// Override this if not relevant.
    async fn health_check(self, addr: &SocketAddr) -> Result<(), error::Error> {
        reqwest::get(reqwest::Url::parse(&format!("http://{addr}/_klyra/healthz")).unwrap())
            .await
