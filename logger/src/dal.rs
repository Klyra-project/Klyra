...
use klyra_proto::logger::{self, LogItem};
use sqlx::{
    ...
    async fn from_pool(pool: SqlitePool) -> Self {
        ...
        tokio::spawn(async move {
            while let Ok(logs) = rx.recv().await {
                let mut builder = QueryBuilder::new("INSERT INTO logs (deployment_id, klyra_service_name, timestamp, level, fields)");
                builder.push_values(logs, |mut b, log| {
                    b.push_bind(log.deployment_id)
                        .push_bind(log.klyra_service_name)
...
pub struct Log {
    ...
    pub(crate) klyra_service_name: String,
...
impl Log {
    ...
    pub fn try_from_scope_span(resource_spans: ResourceSpans) -> Option<Vec<Self>> {
        ...
        let klyra_service_name = get_attribute(resource.clone().attributes, "service.name")?;
        ...
        let logs = scope_spans
            ...
            .flat_map(|scope_spans| {
                ...
                let events: Vec<_> = spans
                    .into_iter()
                    .flat_map(|span| {
                        Self::try_from_span(span, &klyra_service_name, deployment_id.clone())
                    })
                    .flatten()
                    .collect();
    ...
    /// Try to get self from an OTLP [Span]. Also enrich it with the klyra service name and deployment id.
    fn try_from_span(
        span: Span,
        klyra_service_name: &str,
        ...
        let logs = span
            ...
            .flat_map(|event| {
                ...
                Some(Log {
                    klyra_service_name: klyra_service_name.to_string(),
...
impl From<Log> for LogItem {
    fn from(log: Log) -> Self {
        LogItem {
            service_name: log.klyra_service_name,
