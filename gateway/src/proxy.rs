...
impl UserProxy {
    async fn proxy(
        ...
        let span = debug_span!("proxy", http.method = %req.method(), http.host = ?req.headers().get("Host"), http.uri = %req.uri(), http.status_code = field::Empty, klyra.project.name = field::Empty);
