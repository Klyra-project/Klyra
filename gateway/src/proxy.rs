...
use klyra_backends::cache::{CacheManagement, CacheManager};
use klyra_backends::headers::XKlyraProject;
use klyra_backends::project_name::ProjectName;
use klyra_common::constants::DEPLOYER_SERVICE_HTTP_PORT;
...
async fn proxy(
    ...
    let span = debug_span!("proxy", http.method = %req.method(), http.host = field::Empty, http.uri = %req.uri(), http.status_code = field::Empty, klyra.project.name = field::Empty);
    ...
    span.record("klyra.project.name", &project_name.to_string());
