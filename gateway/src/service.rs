...
use klyra_backends::client::{permit, PermissionsDal};
use klyra_backends::headers::XKlyraAdminSecret;
...
use klyra_common::models::user::{AccountTier, UserId};
...
impl GatewayService {
    ...
    pub async fn route(
        ...
        headers.insert("x-klyra-account-name", user_id.parse().unwrap());
    ...
    pub async fn create_project(
        ...
        .await?
        {
            ...
            // 2024.
            Err(Deprecated(
                "New project creation on the klyra.rs platform has been deprecated.".to_string(),
            )
            .into())
        } else {
        ...
        match self.docker_stats_source {
            DockerStatsSource::CgroupV1 => {
                ...
                .map_err(|err| {
                    error!(
                        error = &err as &dyn std::error::Error,
                        klyra.container.id = container_id,
            ...
            DockerStatsSource::CgroupV2 => {
                ...
                .ok_or_else(|| {
                    ...
                    error!(klyra.container.id = container_id, error = &err);
