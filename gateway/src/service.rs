...
use klyra_common::backends::headers::{XKlyraAccountName, XKlyraAdminSecret};
use klyra_common::claims::AccountTier;
use klyra_common::constants::klyra_IDLE_DOCS_URL;
use klyra_common::models::project::{ProjectName, State};
...
impl GatewayService {
    ...
    pub async fn init(
        ...
        if (klyra_env == "staging" || klyra_env == "production")
            && docker_stats_source == DockerStatsSource::Bollard
        {
            return Err(io::Error::new(
                io::ErrorKind::NotFound,
                format!(
                    "klyra_ENV is {} and could not find docker stats at path: {:?} or {:?}",
                    klyra_env, DOCKER_STATS_PATH_CGROUP_V1, DOCKER_STATS_PATH_CGROUP_V2,
                ),
            ));
        }
    ...
    pub async fn route(
        ...
        headers.typed_insert(XKlyraAccountName(account_name.to_string()));
        headers.typed_insert(XKlyraAdminSecret(control_key));
        ...
        .await?
        {
            ...
            if project.is_destroyed() {
                ...
                let message = match state {
                    ...
                    | State::Rebooting
                    | State::Destroying => {
                        format!("project '{project_name}' is already {state}. You can check the status again using `cargo klyra project status`.")
                    }
                    ...
                    State::Started | State::Destroyed => {
                        format!("project '{project_name}' is already {state}. Try using `cargo klyra project restart` instead.")
                    }
                    State::Stopped => {
                        format!("project '{project_name}' is idled. Find out more about idle projects here: {klyra_IDLE_DOCS_URL}")
                    }
    ...
    pub async fn find_or_start_project(
        ...
        if project.state.is_stopped() {
            trace!(klyra.project.name = %project_name, "starting up idle project");
    ...
    pub async fn has_capacity(
        ...
        const CCH_CONTROL_FILE: &str = "/var/lib/klyra/BLOCK_CCH23_PROJECT_TRAFFIC";
...
impl DockerContext for GatewayContext {
    ...
    #[instrument(name = "get container stats", skip_all, fields(docker_stats_source = %self.docker_stats_source, klyra.container.id = container_id))]
    async fn get_stats(&self, container_id: &str) -> Result<u64, Error> {
        match self.docker_stats_source {
            DockerStatsSource::CgroupV1 => {
                ...
                .await.map_err(|e| {
                    error!(error = %e, klyra.container.id = container_id, "failed to read docker stats file for container");
                    ProjectError::internal("failed to read docker stats file for container")
                })?
                .trim()
                .parse()
                .map_err(|e| {
                    error!(error = %e, klyra.container.id = container_id, "failed to parse cpu usage stat");
            ...
            DockerStatsSource::CgroupV2 => {
                ...
                let cpu_usage: u64 = tokio::fs::read_to_string(format!(
                    ...
                    .next()
                    .ok_or_else(|| {
                        error!(klyra.container.id = container_id, "failed to read first line of docker stats file for container");
                        ProjectError::internal("failed to read first line of docker stats file for container")
                    })?
                    .split(' ')
                    .nth(1)
                    .ok_or_else(|| {
                        error!(klyra.container.id = container_id, "failed to split docker stats line for container");
...
impl GatewayContext {
    ...
    pub async fn get_jwt(&self) -> String {
        ...
        headers.typed_insert(XKlyraAdminSecret(self.gateway_api_key.clone()));
