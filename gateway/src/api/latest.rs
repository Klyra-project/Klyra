...
use klyra_backends::client::permit::Team;
...
use klyra_common::models::{service, team};
...
async fn override_create_service(scoped_user: ScopedUser) -> Result<Response<Body>, ApiError> {
    // Creating new deployments on the klyra.rs platform is deprecated as of the first of January
    // 2025.
    return Err(Deprecated(
        "Creating new deployments on the klyra.rs platform has been deprecated.".to_string(),
    )
    .into());
}
...
#[instrument(skip_all, fields(klyra.team.name = %team_name, klyra.team.id = field::Empty))]
async fn create_team(
    ...
    Span::current().record("klyra.team.id", &team.id);
...
#[instrument(skip_all, fields(klyra.team.id = %team_id))]
...
#[instrument(skip_all, fields(klyra.team.id = %team_id, klyra.project.id = %project_id))]
