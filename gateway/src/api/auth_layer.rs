...
impl<S> Layer<S> for KlyraAuthLayer {
    type Service = KlyraAuthService<S>;
    ...
    fn call(&mut self, mut req: Request<Body>) -> Self::Future {
        ...
        if req.uri().path() == "/logout" {
            if let Ok(Some(cookie)) = req.headers().typed_try_get::<Cookie>() {
                if let Some(cache_key) = cookie.get("klyra.sid").map(|id| id.to_string()) {
                    self.cache_manager.invalidate(&cache_key);
                }
            };
        }
...
fn make_token_request(
    ...
    if let Some(key) = gateway_admin_key {
        trace!("XKlyraAdminSecret header inserted for token request");
        token_request
            .headers_mut()
            .expect("manual request to be valid")
            .typed_insert(XKlyraAdminSecret(key.to_string()));
    }
