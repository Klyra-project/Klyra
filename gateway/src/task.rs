...
impl Task<()> for ProjectTask {
    ...
    async fn poll(&mut self, _: ()) -> TaskResult<Self::Output> {
        ...
        let span = info_span!(
            "polling project",
            klyra.project.name = %project_ctx.project_name,
            klyra.operation_name = field::Empty,
        ...
        async {
            ...
            let res = {
                ...
                tokio::select! {
                    ...
                    _ = timeout => {
                        warn!(
                            klyra.project.name = %self.project_name,
                ...
                TaskResult::Done(_) => {
                    ...
                    if self.tasks.is_empty() {
                        ...
                        if let Some(operation) = &self.operation_name {
                            ...
                            Span::current().record("klyra.operation_name", operation);
                        }
                        TaskResult::Done(())
                    } else {
                        TaskResult::Pending(())
