...
pub mod tests {
    ...
    use klyra_common::{DeploymentMeta, DeploymentStateMeta};
    ...
    impl World {
        pub async fn new() -> Self {
            ...
            let image = env::var("klyra_TESTS_RUNTIME_IMAGE")
                .unwrap_or("public.ecr.aws/klyra/backend:latest".to_string());

            let network_name =
                env::var("klyra_TESTS_NETWORK").unwrap_or("klyra_default".to_string());
    ...
    async fn end_to_end() {
        ...
        proxy_client
            .request(
                Request::get("/hello")
                    .header("Host", "matrix.klyraapp.rs")
                    .body(Body::empty())
                    .unwrap(),
            )
            .map_ok(|resp| {
