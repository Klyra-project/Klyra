...
.PHONY: images clean src up down deploy klyra-% klyra-images postgres docker-compose.rendered.yml test bump-% deploy-examples publish publish-% --validate-version

clean:
	rm .klyra-*
	rm docker-compose.rendered.yml

images: klyra-images postgres panamax otel

klyra-images: klyra-auth klyra-deployer klyra-gateway klyra-provisioner klyra-resource-recorder
