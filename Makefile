SRC_CRATES=deployer common codegen cargo-klyra proto provisioner service
...
.PHONY: images clean src up down deploy klyra-% postgres docker-compose.rendered.yml test bump-% deploy-examples publish publish-% --validate-version

clean:
	rm .klyra-*
	rm docker-compose.rendered.yml

images: klyra-provisioner klyra-deployer klyra-gateway klyra-auth klyra-resource-recorder postgres panamax otel
...
up: $(DOCKER_COMPOSE_FILES)
	if [ "$(klyra_DETACH)" = "disable" ]; then $(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE) $(addprefix -f ,$(DOCKER_COMPOSE_FILES)) -p $(STACK) up; else $(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE) $(addprefix -f ,$(DOCKER_COMPOSE_FILES)) -p $(STACK) up --detach; fi

down: $(DOCKER_COMPOSE_FILES)
	$(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE) $(addprefix -f ,$(DOCKER_COMPOSE_FILES)) -p $(STACK) down

klyra-%: ${SRC} Cargo.lock
	$(DOCKER_BUILD) \
		--build-arg folder=$(*) \
...
# Bunch of targets to make bumping the klyra version easier
#
...
# Deploy all our example using the command set in klyra-command
# Usage: make deploy-example klyra-command="cargo klyra" -j 2
...
publish: publish-resources publish-cargo-klyra
