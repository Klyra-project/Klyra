...
# Files and directories used by the Klyra build process:
...
RUN touch /app/Klyra.toml
RUN touch /app/klyra_prebuild.sh
RUN touch /app/klyra_postbuild.sh
RUN touch /app/klyra_setup_container.sh
...
COPY klyra_prebuild.sh .
RUN bash klyra_prebuild.sh
...
RUN bash klyra_postbuild.sh

RUN mv /app/target/release/hello /executable



RUN for path in \$(tq -r '.build.assets // .build_assets // [] | join(" ")' Klyra.toml); do find "\$path" -type f -exec echo Copying \{\} \; -exec install -D \{\} /build_assets/\{\} \; ; done


FROM runtime-base AS runtime
WORKDIR /app

COPY --from=builder /app/klyra_setup_container.sh /tmp
RUN bash /tmp/klyra_setup_container.sh; rm /tmp/klyra_setup_container.sh
