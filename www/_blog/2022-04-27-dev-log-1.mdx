...
In the [previous DevLog](https://www.klyra.rs/blog/2022/04/22/dev-log-0) we started the journey of building the klyra MVP. We went over the design and implementation of the `cargo` subcommand which deploys cargo projects to klyra. This has been a race against the clock, so corners were cut and tradeoffs were made. A similar theme emerges in this DevLog which covers the **deployment state machine**. We're going to think about compiling and deploying user code, while also covering one of my favourite design patterns in Rust.
