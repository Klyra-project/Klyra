...
I'm using [klyra](https://klyra.rs) for this project. It's a serverless platform built for Rust and I don't have to deal with provisioning databases, or subdomains or any of that gunk. I already have the CLI [installed](https://docs.rs/klyra-service/0.2.5/klyra_service/#deploying) and an [account](https://www.klyra.rs/) so I simply:
...
I open up `src/lib.rs` and overwrite it with the `klyra` entrypoint code:
...
Next I import all of the dependencies to get klyra to work with Rocket - pretty simple. I open up `Cargo.toml` add a couple of lines:
...
klyra-service = { version = "0.2", features = ["sqlx-postgres", "web-rocket"] }
```
...
$ cargo klyra deploy
   ...
   Compiling url-shortener v0.1.0 (/opt/klyra/crates/url-shortener)
...
```
$ curl -X https://url-shortener.klyraapp.rs/hello
Hello, world!
```
...
`klyra` does a lot of this stuff for you - I just didn't remember how. I quickly head over to the [klyra / sqlx](https://docs.rs/klyra-service/0.2.5/klyra_service/#using-sqlx) section in the docs. I added the `sqlx` dependency to `Cargo.toml` and change _one line_ in `lib.rs`:
...
By adding a parameter to the main `rocket` function, `klyra` will automatically provision a Postgres database for you, create an account and hand you back an authenticated connection pool which is usable from your application code.
...
The database provisioned by `klyra` is completely empty - I'm going to need to either connect to Postgres and create the schema myself, or write some sort of code to automatically perform the migration. As I start to ponder this seemingly existential question I decide not to overthink it. I'm just going to go with whatever is easiest.

I connect to the database provisioned by klyra using [pgAdmin](https://www.pgadmin.org/) using the provided database URI and run the following script:
...
async fn shorten(url: String, pool: &State<PgPool>) -> Result<String, Status> {
    ...
    Ok(format!("https://url-shortener.klyraapp.rs/{id}"))
}
```
...
After I fixed my typo and sorted out the various unresolved dependencies by letting my IDE do the heavy lifting for me, I deployed to klyra for the last time.

## Moment of truth - 00:25 minutes remaining

Feeling like an off-brand Tom Cruise in mission impossible I stared intently at the clock counting down as klyra deployed my url-shortener. 19.3 seconds and we're live. As soon as the `DEPLOYED` dialog came up, I instantly tested it out:

```bash
$ curl -X POST -d "https://google.com" https://url-shortener.klyraapp.rs
https://s.klyraapp.rs/XDlrTB⏎
```
