...
description: DevLog[0] is the first in a series of posts about how we built the klyra MVP
...
At klyra we think there is a better paradigm for building applications. We call it Infrastructure From Code (IFC).
...
1. A single annotation can transform your web app into a klyra app: `#[klyra_service::main]`
2. You can get started with a single cargo command: `$ cargo klyra deploy`
3. Your klyra app is automatically provisioned a subdomain `my-app.klyraapp.rs`
...
1. `$ cargo klyra deploy` runs a `cargo package` under the hood, zipping up the current cargo project into a tarball and shipping it to our API under the `/deploy` endpoint with a bearer token for authentication
...
5. We update the routing table of our reverse proxy such that requests coming in with the host `my-app.klyraapp.rs` are forwarded to the aforementioned port.

And that's it! It turns out there are more than a few devils in the details here - but that was our plan in all it's glory.

## $ cargo klyra deploy

The cargo subcommand [`cargo klyra`](https://github.com/getsynth/klyra/tree/main/cargo-klyra) seemed like the obvious place to start.

 To create a third-party cargo subcommand, the binary needs to be named `cargo-${command}` and it needs to be stored in `~./.cargo/bin`. The easiest way to do this is to create a binary called `cargo-klyra` and publish it to `crates.io`. Then,  `cargo install cargo-klyra` will place it in `~./.cargo/bin`. Pretty simple.

`cargo-klyra` also needs an HTTP client to make requests against the API, as well as some config logic to hold API keys. Finally, `cargo-klyra` needs to use the `cargo` crate to programmatically run cargo commands like `cargo package`.
...
After a day of grappling and digging into the `cargo` source code, `cargo-klyra` was happily packaging up cargo projects and serializing them nicely into the body of a POST request.

We had built the bare bones of our client, next up was build system.

## Next Steps

In the next devlog we'll be exploring how we hacked together a build system to compile `cdylib`s for them to be dynamically linked to the API runtime. In the meantime, if you want to try out klyra head over to the [getting started](https://docs.rs/klyra-service/0.2.6/klyra_service/) section! It's completely free while klyra is still in Alpha.
