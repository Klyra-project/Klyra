...
Before our app can use the database we have to add sqlx with some features: `cargo add sqlx -F postgres runtime-tokio-native-tls`. We will also add the `klyra-shared-db` crate with `cargo add klyra-shared-db -F postgres`. 

Now back in the code we add a parameter with `#[klyra_shared_db::Postgres] pool: Database`. The `#[klyra_shared_db::Postgres]` annotation tells klyra to provision a Postgres database using the [infrastructure from code design](https://www.klyra.rs/blog/2022/05/09/ifc)!
...
#[klyra_service::main]
async fn axum(
    #[klyra_shared_db::Postgres] pool: Database
) -> KlyraAxum {
