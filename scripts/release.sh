...
function update-examples-versions()
{
    ...
    rg "klyra-service = \{ version" --files-with-matches service/ | xargs sed -i "s/klyra-service = { version = \"[[:digit:]]*.[[:digit:]]*.[[:digit:]]*\"/klyra-service = { version = \"$version\"/g"
    sed -i "s/klyra-service = { version = \"[[:digit:]]*.[[:digit:]]*.[[:digit:]]*\"/klyra-service = { version = \"$version\"/g" README.md
