...
impl Services {
        ...
        C: Into<Color>,
    {
        ...
        Command::new("docker")
            .args(["network", "create", "--driver", "bridge", "klyra-net"])
            .spawn()
            .unwrap()
            .wait()
            .unwrap();
        ...
        let provisioner_container = format!("klyra_provisioner_{}_{}", api.target, api.id);
        ...
        let api_container = format!("klyra_api_{}_{}", api.target, api.id);
        let mut run = Command::new("docker");
        run.args([
            "run",
            "--name",
            ...
            "API_PORT=8001",
            "-e",
            "PROXY_FQDN=klyraapp.test",
            "-e",
            "klyra_USERS_TOML=/config/users.toml",
            "-e",
            "klyra_INITIAL_KEY=ci-test",
            "-e",
    ...
    fn build_image(service: &str, target: &str, id: &str) -> String {
        let image = format!("klyra_{service}_{target}_{id}");
