...
/// The directory given to `cargo klyra` run in the context of E2E
/// testing
...
impl TempCargoHome {
    /// Initialize a new `TempCargoHome` with a `klyra-service`
    /// patch unless `klyra_CARGO_HOME` is set, then use that. With
    /// the latter, no patch is applied
    pub fn from_env_or_new() -> Self {
        match env::var("klyra_CARGO_HOME") {
            Ok(path) => Self::User(path.into()),
            Err(_) => {
                let dir = TempDir::new("klyra-tests").unwrap();
...
",
        ...
        Command::new(CARGO.as_os_str())
            ...
            .current_dir(WORKSPACE_ROOT.as_path())
            .output()
            .ensure_success("failed to `cargo build --bin cargo-klyra`");
        ...
        Command::new(DOCKER.as_os_str())
            .args([
                ...
                "--state=/var/lib/klyra/gateway.sqlite",
                "init",
                "--name",
                "admin",
                "--key",
    ...
    pub fn wait_deployer_ready(&self, project_path: &str, mut timeout: Duration) {
        ...
        while !timeout.is_zero() {
            ...
            if env::var("klyra_API_KEY").is_err() {
                run.env("klyra_API_KEY", "test-key");
            }
